...
(function(){var initializing=false,fnTest=/xyz/.test(function(){xyz})?/\b_super\b/:/.*/;this.Class=function(){};Class.extend=function(prop){var _super=this.prototype;initializing=true;var prototype=new this;initializing=false;for(var name in prop){prototype[name]=typeof prop[name]=="function"&&typeof _super[name]=="function"&&fnTest.test(prop[name])?function(name,fn){return function(){var tmp=this._super;this._super=_super[name];var ret=fn.apply(this,arguments);this._super=tmp;return ret}}(name,prop[name]):prop[name]}function Class(){if(!initializing&&this.init)this.init.apply(this,arguments)}Class.prototype=prototype;Class.prototype.constructor=Class;Class.extend=arguments.callee;return Class}})();function FastClick(layer){"use strict";var oldOnClick;this.trackingClick=false;this.trackingClickStart=0;this.targetElement=null;this.touchStartX=0;this.touchStartY=0;this.lastTouchIdentifier=0;this.touchBoundary=10;this.layer=layer;if(FastClick.notNeeded(layer)){return}function bind(method,context){return function(){return method.apply(context,arguments)}}if(deviceIsAndroid){layer.addEventListener("mouseover",bind(this.onMouse,this),true);layer.addEventListener("mousedown",bind(this.onMouse,this),true);layer.addEventListener("mouseup",bind(this.onMouse,this),true)}layer.addEventListener("click",bind(this.onClick,this),true);layer.addEventListener("touchstart",bind(this.onTouchStart,this),false);layer.addEventListener("touchmove",bind(this.onTouchMove,this),false);layer.addEventListener("touchend",bind(this.onTouchEnd,this),false);layer.addEventListener("touchcancel",bind(this.onTouchCancel,this),false);if(!Event.prototype.stopImmediatePropagation){layer.removeEventListener=function(type,callback,capture){var rmv=Node.prototype.removeEventListener;if(type==="click"){rmv.call(layer,type,callback.hijacked||callback,capture)}else{rmv.call(layer,type,callback,capture)}};layer.addEventListener=function(type,callback,capture){var adv=Node.prototype.addEventListener;if(type==="click"){adv.call(layer,type,callback.hijacked||(callback.hijacked=function(event){if(!event.propagationStopped){callback(event)}}),capture)}else{adv.call(layer,type,callback,capture)}}}if(typeof layer.onclick==="function"){oldOnClick=layer.onclick;layer.addEventListener("click",function(event){oldOnClick(event)},false);layer.onclick=null}}var deviceIsAndroid=navigator.userAgent.indexOf("Android")>0;var deviceIsIOS=/iP(ad|hone|od)/.test(navigator.userAgent);var deviceIsIOS4=deviceIsIOS&&/OS 4_\d(_\d)?/.test(navigator.userAgent);var deviceIsIOSWithBadTarget=deviceIsIOS&&/OS ([6-9]|\d{2})_\d/.test(navigator.userAgent);FastClick.prototype.needsClick=function(target){"use strict";switch(target.nodeName.toLowerCase()){case"button":case"select":case"textarea":if(target.disabled){return true}break;case"input":if(deviceIsIOS&&target.type==="file"||target.disabled){return true}break;case"label":case"video":return true}return/\bneedsclick\b/.test(target.className)};FastClick.prototype.needsFocus=function(target){"use strict";switch(target.nodeName.toLowerCase()){case"textarea":return true;case"select":return!deviceIsAndroid;case"input":switch(target.type){case"button":case"checkbox":case"file":case"image":case"radio":case"submit":return false}return!target.disabled&&!target.readOnly;default:return/\bneedsfocus\b/.test(target.className)}};FastClick.prototype.sendClick=function(targetElement,event){"use strict";var clickEvent,touch;if(document.activeElement&&document.activeElement!==targetElement){document.activeElement.blur()}touch=event.changedTouches[0];clickEvent=document.createEvent("MouseEvents");clickEvent.initMouseEvent(this.determineEventType(targetElement),true,true,window,1,touch.screenX,touch.screenY,touch.clientX,touch.clientY,false,false,false,false,0,null);clickEvent.forwardedTouchEvent=true;targetElement.dispatchEvent(clickEvent)};FastClick.prototype.determineEventType=function(targetElement){"use strict";if(deviceIsAndroid&&targetElement.tagName.toLowerCase()==="select"){return"mousedown"}return"click"};FastClick.prototype.focus=function(targetElement){"use strict";var length;if(deviceIsIOS&&targetElement.setSelectionRange&&targetElement.type.indexOf("date")!==0&&targetElement.type!=="time"){length=targetElement.value.length;targetElement.setSelectionRange(length,length)}else{targetElement.focus()}};FastClick.prototype.updateScrollParent=function(targetElement){"use strict";var scrollParent,parentElement;scrollParent=targetElement.fastClickScrollParent;if(!scrollParent||!scrollParent.contains(targetElement)){parentElement=targetElement;do{if(parentElement.scrollHeight>parentElement.offsetHeight){scrollParent=parentElement;targetElement.fastClickScrollParent=parentElement;break}parentElement=parentElement.parentElement}while(parentElement)}if(scrollParent){scrollParent.fastClickLastScrollTop=scrollParent.scrollTop}};FastClick.prototype.getTargetElementFromEventTarget=function(eventTarget){"use strict";if(eventTarget.nodeType===Node.TEXT_NODE){return eventTarget.parentNode}return eventTarget};FastClick.prototype.onTouchStart=function(event){"use strict";var targetElement,touch,selection;if(event.targetTouches.length>1){return true}targetElement=this.getTargetElementFromEventTarget(event.target);touch=event.targetTouches[0];if(deviceIsIOS){selection=window.getSelection();if(selection.rangeCount&&!selection.isCollapsed){return true}if(!deviceIsIOS4){if(touch.identifier===this.lastTouchIdentifier){event.preventDefault();return false}this.lastTouchIdentifier=touch.identifier;this.updateScrollParent(targetElement)}}this.trackingClick=true;this.trackingClickStart=event.timeStamp;this.targetElement=targetElement;this.touchStartX=touch.pageX;this.touchStartY=touch.pageY;if(event.timeStamp-this.lastClickTime<200){event.preventDefault()}return true};FastClick.prototype.touchHasMoved=function(event){"use strict";var touch=event.changedTouches[0],boundary=this.touchBoundary;if(Math.abs(touch.pageX-this.touchStartX)>boundary||Math.abs(touch.pageY-this.touchStartY)>boundary){return true}return false};FastClick.prototype.onTouchMove=function(event){"use strict";if(!this.trackingClick){return true}if(this.targetElement!==this.getTargetElementFromEventTarget(event.target)||this.touchHasMoved(event)){this.trackingClick=false;this.targetElement=null}return true};FastClick.prototype.findControl=function(labelElement){"use strict";if(labelElement.control!==undefined){return labelElement.control}if(labelElement.htmlFor){return document.getElementById(labelElement.htmlFor)}return labelElement.querySelector("button, input:not([type=hidden]), keygen, meter, output, progress, select, textarea")};FastClick.prototype.onTouchEnd=function(event){"use strict";var forElement,trackingClickStart,targetTagName,scrollParent,touch,targetElement=this.targetElement;if(!this.trackingClick){return true}if(event.timeStamp-this.lastClickTime<200){this.cancelNextClick=true;return true}this.cancelNextClick=false;this.lastClickTime=event.timeStamp;trackingClickStart=this.trackingClickStart;this.trackingClick=false;this.trackingClickStart=0;if(deviceIsIOSWithBadTarget){touch=event.changedTouches[0];targetElement=document.elementFromPoint(touch.pageX-window.pageXOffset,touch.pageY-window.pageYOffset)||targetElement;targetElement.fastClickScrollParent=this.targetElement.fastClickScrollParent}targetTagName=targetElement.tagName.toLowerCase();if(targetTagName==="label"){forElement=this.findControl(targetElement);if(forElement){this.focus(targetElement);if(deviceIsAndroid){return false}targetElement=forElement}}else if(this.needsFocus(targetElement)){if(event.timeStamp-trackingClickStart>100||deviceIsIOS&&window.top!==window&&targetTagName==="input"){this.targetElement=null;return false}this.focus(targetElement);this.sendClick(targetElement,event);if(!deviceIsIOS4||targetTagName!=="select"){this.targetElement=null;event.preventDefault()}return false}if(deviceIsIOS&&!deviceIsIOS4){scrollParent=targetElement.fastClickScrollParent;if(scrollParent&&scrollParent.fastClickLastScrollTop!==scrollParent.scrollTop){return true}}if(!this.needsClick(targetElement)){event.preventDefault();this.sendClick(targetElement,event)}return false};FastClick.prototype.onTouchCancel=function(){"use strict";this.trackingClick=false;this.targetElement=null};FastClick.prototype.onMouse=function(event){"use strict";if(!this.targetElement){return true}if(event.forwardedTouchEvent){return true}if(!event.cancelable){return true}if(!this.needsClick(this.targetElement)||this.cancelNextClick){if(event.stopImmediatePropagation){event.stopImmediatePropagation()}else{event.propagationStopped=true}event.stopPropagation();event.preventDefault();return false}return true};FastClick.prototype.onClick=function(event){"use strict";var permitted;if(this.trackingClick){this.targetElement=null;this.trackingClick=false;return true}if(event.target.type==="submit"&&event.detail===0){return true}permitted=this.onMouse(event);if(!permitted){this.targetElement=null}return permitted};FastClick.prototype.destroy=function(){"use strict";var layer=this.layer;if(deviceIsAndroid){layer.removeEventListener("mouseover",this.onMouse,true);layer.removeEventListener("mousedown",this.onMouse,true);layer.removeEventListener("mouseup",this.onMouse,true)}layer.removeEventListener("click",this.onClick,true);layer.removeEventListener("touchstart",this.onTouchStart,false);layer.removeEventListener("touchmove",this.onTouchMove,false);layer.removeEventListener("touchend",this.onTouchEnd,false);layer.removeEventListener("touchcancel",this.onTouchCancel,false)};FastClick.notNeeded=function(layer){"use strict";var metaViewport;var chromeVersion;if(typeof window.ontouchstart==="undefined"){return true}chromeVersion=+(/Chrome\/([0-9]+)/.exec(navigator.userAgent)||[,0])[1];if(chromeVersion){if(deviceIsAndroid){metaViewport=document.querySelector("meta[name=viewport]");if(metaViewport){if(metaViewport.content.indexOf("user-scalable=no")!==-1){return true}if(chromeVersion>31&&window.innerWidth<=window.screen.width){return true}}}else{return true}}if(layer.style.msTouchAction==="none"){return true}return false};FastClick.attach=function(layer){"use strict";return new FastClick(layer)};if(typeof define!=="undefined"&&define.amd){define(function(){"use strict";return FastClick})}else if(typeof module!=="undefined"&&module.exports){module.exports=FastClick.attach;module.exports.FastClick=FastClick}else{window.FastClick=FastClick}(function(window,undefined){"use strict";var Hammer=function(element,options){return new Hammer.Instance(element,options||{})};Hammer.defaults={stop_browser_behavior:{userSelect:"none",touchAction:"none",touchCallout:"none",contentZooming:"none",userDrag:"none",tapHighlightColor:"rgba(0,0,0,0)"}};Hammer.HAS_POINTEREVENTS=window.navigator.pointerEnabled||window.navigator.msPointerEnabled;Hammer.HAS_TOUCHEVENTS="ontouchstart"in window;Hammer.MOBILE_REGEX=/mobile|tablet|ip(ad|hone|od)|android|silk/i;Hammer.NO_MOUSEEVENTS=Hammer.HAS_TOUCHEVENTS&&window.navigator.userAgent.match(Hammer.MOBILE_REGEX);Hammer.EVENT_TYPES={};Hammer.DIRECTION_DOWN="down";Hammer.DIRECTION_LEFT="left";Hammer.DIRECTION_UP="up";Hammer.DIRECTION_RIGHT="right";Hammer.POINTER_MOUSE="mouse";Hammer.POINTER_TOUCH="touch";Hammer.POINTER_PEN="pen";Hammer.EVENT_START="start";Hammer.EVENT_MOVE="move";Hammer.EVENT_END="end";Hammer.DOCUMENT=window.document;Hammer.plugins=Hammer.plugins||{};Hammer.gestures=Hammer.gestures||{};Hammer.READY=false;function setup(){if(Hammer.READY){return}Hammer.event.determineEventTypes();Hammer.utils.each(Hammer.gestures,function(gesture){Hammer.detection.register(gesture)});Hammer.event.onTouch(Hammer.DOCUMENT,Hammer.EVENT_MOVE,Hammer.detection.detect);Hammer.event.onTouch(Hammer.DOCUMENT,Hammer.EVENT_END,Hammer.detection.detect);Hammer.READY=true}Hammer.utils={extend:function extend(dest,src,merge){for(var key in src){if(dest[key]!==undefined&&merge){continue}dest[key]=src[key]}return dest},each:function(obj,iterator,context){var i,length;if("forEach"in obj){obj.forEach(iterator,context)}else if(obj.length!==undefined){for(i=0,length=obj.length;i<length;i++){if(iterator.call(context,obj[i],i,obj)===false){return}}}else{for(i in obj){if(obj.hasOwnProperty(i)&&iterator.call(context,obj[i],i,obj)===false){return}}}},hasParent:function(node,parent){while(node){if(node==parent){return true}node=node.parentNode}return false},getCenter:function getCenter(touches){var valuesX=[],valuesY=[];Hammer.utils.each(touches,function(touch){valuesX.push(typeof touch.clientX!=="undefined"?touch.clientX:touch.pageX);valuesY.push(typeof touch.clientY!=="undefined"?touch.clientY:touch.pageY)});return{pageX:(Math.min.apply(Math,valuesX)+Math.max.apply(Math,valuesX))/2,pageY:(Math.min.apply(Math,valuesY)+Math.max.apply(Math,valuesY))/2}},getVelocity:function getVelocity(delta_time,delta_x,delta_y){return{x:Math.abs(delta_x/delta_time)||0,y:Math.abs(delta_y/delta_time)||0}},getAngle:function getAngle(touch1,touch2){var y=touch2.pageY-touch1.pageY,x=touch2.pageX-touch1.pageX;return Math.atan2(y,x)*180/Math.PI},getDirection:function getDirection(touch1,touch2){var x=Math.abs(touch1.pageX-touch2.pageX),y=Math.abs(touch1.pageY-touch2.pageY);if(x>=y){return touch1.pageX-touch2.pageX>0?Hammer.DIRECTION_LEFT:Hammer.DIRECTION_RIGHT}else{return touch1.pageY-touch2.pageY>0?Hammer.DIRECTION_UP:Hammer.DIRECTION_DOWN}},getDistance:function getDistance(touch1,touch2){var x=touch2.pageX-touch1.pageX,y=touch2.pageY-touch1.pageY;return Math.sqrt(x*x+y*y)},getScale:function getScale(start,end){if(start.length>=2&&end.length>=2){return this.getDistance(end[0],end[1])/this.getDistance(start[0],start[1])}return 1},getRotation:function getRotation(start,end){if(start.length>=2&&end.length>=2){return this.getAngle(end[1],end[0])-this.getAngle(start[1],start[0])}return 0},isVertical:function isVertical(direction){return direction==Hammer.DIRECTION_UP||direction==Hammer.DIRECTION_DOWN},stopDefaultBrowserBehavior:function stopDefaultBrowserBehavior(element,css_props){if(!css_props||!element||!element.style){return}Hammer.utils.each(["webkit","khtml","moz","Moz","ms","o",""],function(vendor){Hammer.utils.each(css_props,function(prop){if(vendor){prop=vendor+prop.substring(0,1).toUpperCase()+prop.substring(1)}if(prop in element.style){element.style[prop]=prop}})});if(css_props.userSelect=="none"){element.onselectstart=function(){return false}}if(css_props.userDrag=="none"){element.ondragstart=function(){return false}}}};Hammer.Instance=function(element,options){var self=this;setup();this.element=element;this.enabled=true;this.options=Hammer.utils.extend(Hammer.utils.extend({},Hammer.defaults),options||{});if(this.options.stop_browser_behavior){Hammer.utils.stopDefaultBrowserBehavior(this.element,this.options.stop_browser_behavior)}Hammer.event.onTouch(element,Hammer.EVENT_START,function(ev){if(self.enabled){Hammer.detection.startDetect(self,ev)}});return this};Hammer.Instance.prototype={on:function onEvent(gesture,handler){var gestures=gesture.split(" ");Hammer.utils.each(gestures,function(gesture){this.element.addEventListener(gesture,handler,false)},this);return this},off:function offEvent(gesture,handler){var gestures=gesture.split(" ");Hammer.utils.each(gestures,function(gesture){this.element.removeEventListener(gesture,handler,false)},this);return this},trigger:function triggerEvent(gesture,eventData){if(!eventData){eventData={}}var event=Hammer.DOCUMENT.createEvent("Event");event.initEvent(gesture,true,true);event.gesture=eventData;var element=this.element;if(Hammer.utils.hasParent(eventData.target,element)){element=eventData.target}element.dispatchEvent(event);return this},enable:function enable(state){this.enabled=state;return this}};var last_move_event=null;var enable_detect=false;var touch_triggered=false;Hammer.event={bindDom:function(element,type,handler){var types=type.split(" ");Hammer.utils.each(types,function(type){element.addEventListener(type,handler,false)})},onTouch:function onTouch(element,eventType,handler){var self=this;this.bindDom(element,Hammer.EVENT_TYPES[eventType],function bindDomOnTouch(ev){var sourceEventType=ev.type.toLowerCase();if(sourceEventType.match(/mouse/)&&touch_triggered){return}else if(sourceEventType.match(/touch/)||sourceEventType.match(/pointerdown/)||sourceEventType.match(/mouse/)&&ev.which===1){enable_detect=true}else if(sourceEventType.match(/mouse/)&&!ev.which){enable_detect=false}if(sourceEventType.match(/touch|pointer/)){touch_triggered=true}var count_touches=0;if(enable_detect){if(Hammer.HAS_POINTEREVENTS&&eventType!=Hammer.EVENT_END){count_touches=Hammer.PointerEvent.updatePointer(eventType,ev)}else if(sourceEventType.match(/touch/)){count_touches=ev.touches.length}else if(!touch_triggered){count_touches=sourceEventType.match(/up/)?0:1}if(count_touches>0&&eventType==Hammer.EVENT_END){eventType=Hammer.EVENT_MOVE}else if(!count_touches){eventType=Hammer.EVENT_END}if(count_touches||last_move_event===null){last_move_event=ev}handler.call(Hammer.detection,self.collectEventData(element,eventType,self.getTouchList(last_move_event,eventType),ev));if(Hammer.HAS_POINTEREVENTS&&eventType==Hammer.EVENT_END){count_touches=Hammer.PointerEvent.updatePointer(eventType,ev)}}if(!count_touches){last_move_event=null;enable_detect=false;touch_triggered=false;Hammer.PointerEvent.reset()}})},determineEventTypes:function determineEventTypes(){var types;if(Hammer.HAS_POINTEREVENTS){types=Hammer.PointerEvent.getEvents()}else if(Hammer.NO_MOUSEEVENTS){types=["touchstart","touchmove","touchend touchcancel"]}else{types=["touchstart mousedown","touchmove mousemove","touchend touchcancel mouseup"]}Hammer.EVENT_TYPES[Hammer.EVENT_START]=types[0];Hammer.EVENT_TYPES[Hammer.EVENT_MOVE]=types[1];Hammer.EVENT_TYPES[Hammer.EVENT_END]=types[2]},getTouchList:function getTouchList(ev){if(Hammer.HAS_POINTEREVENTS){return Hammer.PointerEvent.getTouchList()}else if(ev.touches){return ev.touches}else{ev.identifier=1;return[ev]}},collectEventData:function collectEventData(element,eventType,touches,ev){var pointerType=Hammer.POINTER_TOUCH;if(ev.type.match(/mouse/)||Hammer.PointerEvent.matchType(Hammer.POINTER_MOUSE,ev)){pointerType=Hammer.POINTER_MOUSE}return{center:Hammer.utils.getCenter(touches),timeStamp:(new Date).getTime(),target:ev.target,touches:touches,eventType:eventType,pointerType:pointerType,srcEvent:ev,preventDefault:function(){if(this.srcEvent.preventManipulation){this.srcEvent.preventManipulation()}if(this.srcEvent.preventDefault){this.srcEvent.preventDefault()}},stopPropagation:function(){this.srcEvent.stopPropagation()},stopDetect:function(){return Hammer.detection.stopDetect()}}}};Hammer.PointerEvent={pointers:{},getTouchList:function(){var self=this;var touchlist=[];Hammer.utils.each(self.pointers,function(pointer){touchlist.push(pointer)});return touchlist},updatePointer:function(type,pointerEvent){if(type==Hammer.EVENT_END){this.pointers={}}else{pointerEvent.identifier=pointerEvent.pointerId;this.pointers[pointerEvent.pointerId]=pointerEvent}return Object.keys(this.pointers).length},matchType:function(pointerType,ev){if(!ev.pointerType){return false}var pt=ev.pointerType,types={};types[Hammer.POINTER_MOUSE]=pt===ev.MSPOINTER_TYPE_MOUSE||pt===Hammer.POINTER_MOUSE;types[Hammer.POINTER_TOUCH]=pt===ev.MSPOINTER_TYPE_TOUCH||pt===Hammer.POINTER_TOUCH;types[Hammer.POINTER_PEN]=pt===ev.MSPOINTER_TYPE_PEN||pt===Hammer.POINTER_PEN;return types[pointerType]},getEvents:function(){return["pointerdown MSPointerDown","pointermove MSPointerMove","pointerup pointercancel MSPointerUp MSPointerCancel"]},reset:function(){this.pointers={}}};Hammer.detection={gestures:[],current:null,previous:null,stopped:false,startDetect:function startDetect(inst,eventData){if(this.current){return}this.stopped=false;this.current={inst:inst,startEvent:Hammer.utils.extend({},eventData),lastEvent:false,name:""};this.detect(eventData)},detect:function detect(eventData){if(!this.current||this.stopped){return}eventData=this.extendEventData(eventData);var inst_options=this.current.inst.options;Hammer.utils.each(this.gestures,function(gesture){if(!this.stopped&&inst_options[gesture.name]!==false){if(gesture.handler.call(gesture,eventData,this.current.inst)===false){this.stopDetect();return false}}},this);if(this.current){this.current.lastEvent=eventData}if(eventData.eventType==Hammer.EVENT_END&&!eventData.touches.length-1){this.stopDetect()}return eventData},stopDetect:function stopDetect(){this.previous=Hammer.utils.extend({},this.current);this.current=null;this.stopped=true},extendEventData:function extendEventData(ev){var startEv=this.current.startEvent;if(startEv&&(ev.touches.length!=startEv.touches.length||ev.touches===startEv.touches)){startEv.touches=[];Hammer.utils.each(ev.touches,function(touch){startEv.touches.push(Hammer.utils.extend({},touch))})}var delta_time=ev.timeStamp-startEv.timeStamp,delta_x=ev.center.pageX-startEv.center.pageX,delta_y=ev.center.pageY-startEv.center.pageY,velocity=Hammer.utils.getVelocity(delta_time,delta_x,delta_y),interimAngle,interimDirection;if(ev.eventType==="end"){interimAngle=this.current.lastEvent&&this.current.lastEvent.interimAngle;interimDirection=this.current.lastEvent&&this.current.lastEvent.interimDirection}else{interimAngle=this.current.lastEvent&&Hammer.utils.getAngle(this.current.lastEvent.center,ev.center);interimDirection=this.current.lastEvent&&Hammer.utils.getDirection(this.current.lastEvent.center,ev.center)}Hammer.utils.extend(ev,{deltaTime:delta_time,deltaX:delta_x,deltaY:delta_y,velocityX:velocity.x,velocityY:velocity.y,distance:Hammer.utils.getDistance(startEv.center,ev.center),angle:Hammer.utils.getAngle(startEv.center,ev.center),interimAngle:interimAngle,direction:Hammer.utils.getDirection(startEv.center,ev.center),interimDirection:interimDirection,scale:Hammer.utils.getScale(startEv.touches,ev.touches),rotation:Hammer.utils.getRotation(startEv.touches,ev.touches),startEvent:startEv});return ev},register:function register(gesture){var options=gesture.defaults||{};if(options[gesture.name]===undefined){options[gesture.name]=true}Hammer.utils.extend(Hammer.defaults,options,true);gesture.index=gesture.index||1e3;this.gestures.push(gesture);this.gestures.sort(function(a,b){if(a.index<b.index){return-1}if(a.index>b.index){return 1}return 0});return this.gestures}};Hammer.gestures.Drag={name:"drag",index:50,defaults:{drag_min_distance:10,correct_for_drag_min_distance:true,drag_max_touches:1,drag_block_horizontal:false,drag_block_vertical:false,drag_lock_to_axis:false,drag_lock_min_distance:25},triggered:false,handler:function dragGesture(ev,inst){if(Hammer.detection.current.name!=this.name&&this.triggered){inst.trigger(this.name+"end",ev);this.triggered=false;return}if(inst.options.drag_max_touches>0&&ev.touches.length>inst.options.drag_max_touches){return}switch(ev.eventType){case Hammer.EVENT_START:this.triggered=false;break;case Hammer.EVENT_MOVE:if(ev.distance<inst.options.drag_min_distance&&Hammer.detection.current.name!=this.name){return}if(Hammer.detection.current.name!=this.name){Hammer.detection.current.name=this.name;if(inst.options.correct_for_drag_min_distance&&ev.distance>0){var factor=Math.abs(inst.options.drag_min_distance/ev.distance);Hammer.detection.current.startEvent.center.pageX+=ev.deltaX*factor;Hammer.detection.current.startEvent.center.pageY+=ev.deltaY*factor;ev=Hammer.detection.extendEventData(ev)}}if(Hammer.detection.current.lastEvent.drag_locked_to_axis||inst.options.drag_lock_to_axis&&inst.options.drag_lock_min_distance<=ev.distance){ev.drag_locked_to_axis=true}var last_direction=Hammer.detection.current.lastEvent.direction;if(ev.drag_locked_to_axis&&last_direction!==ev.direction){if(Hammer.utils.isVertical(last_direction)){ev.direction=ev.deltaY<0?Hammer.DIRECTION_UP:Hammer.DIRECTION_DOWN}else{ev.direction=ev.deltaX<0?Hammer.DIRECTION_LEFT:Hammer.DIRECTION_RIGHT}}if(!this.triggered){inst.trigger(this.name+"start",ev);this.triggered=true}inst.trigger(this.name,ev);inst.trigger(this.name+ev.direction,ev);if(inst.options.drag_block_vertical&&Hammer.utils.isVertical(ev.direction)||inst.options.drag_block_horizontal&&!Hammer.utils.isVertical(ev.direction)){ev.preventDefault()}break;case Hammer.EVENT_END:if(this.triggered){inst.trigger(this.name+"end",ev)}this.triggered=false;break}}};Hammer.gestures.Hold={name:"hold",index:10,defaults:{hold_timeout:500,hold_threshold:1},timer:null,handler:function holdGesture(ev,inst){switch(ev.eventType){case Hammer.EVENT_START:clearTimeout(this.timer);Hammer.detection.current.name=this.name;this.timer=setTimeout(function(){if(Hammer.detection.current.name=="hold"){inst.trigger("hold",ev)}},inst.options.hold_timeout);break;case Hammer.EVENT_MOVE:if(ev.distance>inst.options.hold_threshold){clearTimeout(this.timer)}break;case Hammer.EVENT_END:clearTimeout(this.timer);break}}};Hammer.gestures.Release={name:"release",index:Infinity,handler:function releaseGesture(ev,inst){if(ev.eventType==Hammer.EVENT_END){inst.trigger(this.name,ev)}}};Hammer.gestures.Swipe={name:"swipe",index:40,defaults:{swipe_min_touches:1,swipe_max_touches:1,swipe_velocity:.7},handler:function swipeGesture(ev,inst){if(ev.eventType==Hammer.EVENT_END){if(inst.options.swipe_max_touches>0&&ev.touches.length<inst.options.swipe_min_touches&&ev.touches.length>inst.options.swipe_max_touches){return}if(ev.velocityX>inst.options.swipe_velocity||ev.velocityY>inst.options.swipe_velocity){inst.trigger(this.name,ev);inst.trigger(this.name+ev.direction,ev)}}}};Hammer.gestures.Tap={name:"tap",index:100,defaults:{tap_max_touchtime:250,tap_max_distance:10,tap_always:true,doubletap_distance:20,doubletap_interval:300},handler:function tapGesture(ev,inst){if(ev.eventType==Hammer.EVENT_END&&ev.srcEvent.type!="touchcancel"){var prev=Hammer.detection.previous,did_doubletap=false;if(ev.deltaTime>inst.options.tap_max_touchtime||ev.distance>inst.options.tap_max_distance){return}if(prev&&prev.name=="tap"&&ev.timeStamp-prev.lastEvent.timeStamp<inst.options.doubletap_interval&&ev.distance<inst.options.doubletap_distance){inst.trigger("doubletap",ev);did_doubletap=true}if(!did_doubletap||inst.options.tap_always){Hammer.detection.current.name="tap";inst.trigger(Hammer.detection.current.name,ev)}}}};Hammer.gestures.Touch={name:"touch",index:-Infinity,defaults:{prevent_default:false,prevent_mouseevents:false},handler:function touchGesture(ev,inst){if(inst.options.prevent_mouseevents&&ev.pointerType==Hammer.POINTER_MOUSE){ev.stopDetect();return}if(inst.options.prevent_default){ev.preventDefault()}if(ev.eventType==Hammer.EVENT_START){inst.trigger(this.name,ev)}}};Hammer.gestures.Transform={name:"transform",index:45,defaults:{transform_min_scale:.01,transform_min_rotation:1,transform_always_block:false},triggered:false,handler:function transformGesture(ev,inst){if(Hammer.detection.current.name!=this.name&&this.triggered){inst.trigger(this.name+"end",ev);this.triggered=false;return}if(ev.touches.length<2){return}if(inst.options.transform_always_block){ev.preventDefault()}switch(ev.eventType){case Hammer.EVENT_START:this.triggered=false;break;case Hammer.EVENT_MOVE:var scale_threshold=Math.abs(1-ev.scale);var rotation_threshold=Math.abs(ev.rotation);if(scale_threshold<inst.options.transform_min_scale&&rotation_threshold<inst.options.transform_min_rotation){return}Hammer.detection.current.name=this.name;if(!this.triggered){inst.trigger(this.name+"start",ev);this.triggered=true}inst.trigger(this.name,ev);if(rotation_threshold>inst.options.transform_min_rotation){inst.trigger("rotate",ev)}if(scale_threshold>inst.options.transform_min_scale){inst.trigger("pinch",ev);inst.trigger("pinch"+(ev.scale<1?"in":"out"),ev)}break;case Hammer.EVENT_END:if(this.triggered){inst.trigger(this.name+"end",ev)}this.triggered=false;break}}};window.Hammer=Hammer})(this);var IScroll=function(window,document,Math){var rAF=window.requestAnimationFrame||window.webkitRequestAnimationFrame||window.mozRequestAnimationFrame||window.oRequestAnimationFrame||window.msRequestAnimationFrame||function(callback){window.setTimeout(callback,1e3/60)};var utils=function(){var me={};var _elementStyle=document.createElement("div").style;var _vendor=function(){var vendors=["t","webkitT","MozT","msT","OT"],transform,i=0,l=vendors.length;for(;i<l;i++){transform=vendors[i]+"ransform";if(transform in _elementStyle)return vendors[i].substr(0,vendors[i].length-1)}return false}();function _prefixStyle(style){if(_vendor===false)return false;if(_vendor==="")return style;return _vendor+style.charAt(0).toUpperCase()+style.substr(1)}me.getTime=Date.now||function getTime(){return(new Date).getTime()};me.extend=function(target,obj){for(var i in obj){target[i]=obj[i]}};me.addEvent=function(el,type,fn,capture){el.addEventListener(type,fn,!!capture)};me.removeEvent=function(el,type,fn,capture){el.removeEventListener(type,fn,!!capture)};me.momentum=function(current,start,time,lowerMargin,wrapperSize){var distance=current-start,speed=Math.abs(distance)/time,destination,duration,deceleration=6e-4;destination=current+speed*speed/(2*deceleration)*(distance<0?-1:1);duration=speed/deceleration;if(destination<lowerMargin){destination=wrapperSize?lowerMargin-wrapperSize/2.5*(speed/8):lowerMargin;distance=Math.abs(destination-current);duration=distance/speed}else if(destination>0){destination=wrapperSize?wrapperSize/2.5*(speed/8):0;distance=Math.abs(current)+destination;duration=distance/speed}return{destination:Math.round(destination),duration:duration}};var _transform=_prefixStyle("transform");me.extend(me,{hasTransform:_transform!==false,hasPerspective:_prefixStyle("perspective")in _elementStyle,hasTouch:"ontouchstart"in window,hasPointer:navigator.msPointerEnabled,hasTransition:_prefixStyle("transition")in _elementStyle});me.isAndroidBrowser=/Android/.test(window.navigator.appVersion)&&/Version\/\d/.test(window.navigator.appVersion);me.extend(me.style={},{transform:_transform,transitionTimingFunction:_prefixStyle("transitionTimingFunction"),transitionDuration:_prefixStyle("transitionDuration"),transformOrigin:_prefixStyle("transformOrigin")});me.hasClass=function(e,c){var re=new RegExp("(^|\\s)"+c+"(\\s|$)");return re.test(e.className)};me.addClass=function(e,c){if(me.hasClass(e,c)){return}var newclass=e.className.split(" ");newclass.push(c);e.className=newclass.join(" ")};me.removeClass=function(e,c){if(!me.hasClass(e,c)){return}var re=new RegExp("(^|\\s)"+c+"(\\s|$)","g");e.className=e.className.replace(re," ")};me.offset=function(el){var left=-el.offsetLeft,top=-el.offsetTop;while(el=el.offsetParent){left-=el.offsetLeft;top-=el.offsetTop}return{left:left,top:top}};me.preventDefaultException=function(el,exceptions){for(var i in exceptions){if(exceptions[i].test(el[i])){return true}}return false};me.extend(me.eventType={},{touchstart:1,touchmove:1,touchend:1,mousedown:2,mousemove:2,mouseup:2,MSPointerDown:3,MSPointerMove:3,MSPointerUp:3});me.extend(me.ease={},{quadratic:{style:"cubic-bezier(0.25, 0.46, 0.45, 0.94)",fn:function(k){return k*(2-k)}},circular:{style:"cubic-bezier(0.1, 0.57, 0.1, 1)",fn:function(k){return Math.sqrt(1- --k*k)}},back:{style:"cubic-bezier(0.175, 0.885, 0.32, 1.275)",fn:function(k){var b=4;return(k=k-1)*k*((b+1)*k+b)+1}},bounce:{style:"",fn:function(k){if((k/=1)<1/2.75){return 7.5625*k*k}else if(k<2/2.75){return 7.5625*(k-=1.5/2.75)*k+.75}else if(k<2.5/2.75){return 7.5625*(k-=2.25/2.75)*k+.9375}else{return 7.5625*(k-=2.625/2.75)*k+.984375}}},elastic:{style:"",fn:function(k){var f=.22,e=.4;if(k===0){return 0}if(k==1){return 1}return e*Math.pow(2,-10*k)*Math.sin((k-f/4)*(2*Math.PI)/f)+1}}});me.tap=function(e,eventName){var ev=document.createEvent("Event");ev.initEvent(eventName,true,true);ev.pageX=e.pageX;ev.pageY=e.pageY;e.target.dispatchEvent(ev)};me.click=function(e){var target=e.target,ev;if(target.tagName!="SELECT"&&target.tagName!="INPUT"&&target.tagName!="TEXTAREA"){ev=document.createEvent("MouseEvents");ev.initMouseEvent("click",true,true,e.view,1,target.screenX,target.screenY,target.clientX,target.clientY,e.ctrlKey,e.altKey,e.shiftKey,e.metaKey,0,null);
ev._constructed=true;target.dispatchEvent(ev)}};return me}();function IScroll(el,options){this.wrapper=typeof el=="string"?document.querySelector(el):el;this.scroller=this.wrapper.children[0];this.scrollerStyle=this.scroller.style;this.options={startX:0,startY:0,scrollY:true,directionLockThreshold:5,momentum:true,bounce:true,bounceTime:600,bounceEasing:"",preventDefault:true,preventDefaultException:{tagName:/^(INPUT|TEXTAREA|BUTTON|SELECT)$/},HWCompositing:true,useTransition:true,useTransform:true};for(var i in options){this.options[i]=options[i]}this.translateZ=this.options.HWCompositing&&utils.hasPerspective?" translateZ(0)":"";this.options.useTransition=utils.hasTransition&&this.options.useTransition;this.options.useTransform=utils.hasTransform&&this.options.useTransform;this.options.eventPassthrough=this.options.eventPassthrough===true?"vertical":this.options.eventPassthrough;this.options.preventDefault=!this.options.eventPassthrough&&this.options.preventDefault;this.options.scrollY=this.options.eventPassthrough=="vertical"?false:this.options.scrollY;this.options.scrollX=this.options.eventPassthrough=="horizontal"?false:this.options.scrollX;this.options.freeScroll=this.options.freeScroll&&!this.options.eventPassthrough;this.options.directionLockThreshold=this.options.eventPassthrough?0:this.options.directionLockThreshold;this.options.bounceEasing=typeof this.options.bounceEasing=="string"?utils.ease[this.options.bounceEasing]||utils.ease.circular:this.options.bounceEasing;this.options.resizePolling=this.options.resizePolling===undefined?60:this.options.resizePolling;if(this.options.tap===true){this.options.tap="tap"}this.x=0;this.y=0;this.directionX=0;this.directionY=0;this._events={};this._init();this.refresh();this.scrollTo(this.options.startX,this.options.startY);this.enable()}IScroll.prototype={version:"5.0.6",_init:function(){this._initEvents()},destroy:function(){this._initEvents(true);this._execEvent("destroy")},_transitionEnd:function(e){if(e.target!=this.scroller){return}this._transitionTime(0);if(!this.resetPosition(this.options.bounceTime)){this._execEvent("scrollEnd")}},_start:function(e){if(utils.eventType[e.type]!=1){if(e.button!==0){return}}if(!this.enabled||this.initiated&&utils.eventType[e.type]!==this.initiated){return}if(this.options.preventDefault&&!utils.isAndroidBrowser&&!utils.preventDefaultException(e.target,this.options.preventDefaultException)){e.preventDefault()}var point=e.touches?e.touches[0]:e,pos;this.initiated=utils.eventType[e.type];this.moved=false;this.distX=0;this.distY=0;this.directionX=0;this.directionY=0;this.directionLocked=0;this._transitionTime();this.isAnimating=false;this.startTime=utils.getTime();if(this.options.useTransition&&this.isInTransition){pos=this.getComputedPosition();this._translate(Math.round(pos.x),Math.round(pos.y));this._execEvent("scrollEnd");this.isInTransition=false}this.startX=this.x;this.startY=this.y;this.absStartX=this.x;this.absStartY=this.y;this.pointX=point.pageX;this.pointY=point.pageY;this._execEvent("beforeScrollStart")},_move:function(e){if(!this.enabled||utils.eventType[e.type]!==this.initiated){return}if(this.options.preventDefault){e.preventDefault()}var point=e.touches?e.touches[0]:e,deltaX=point.pageX-this.pointX,deltaY=point.pageY-this.pointY,timestamp=utils.getTime(),newX,newY,absDistX,absDistY;this.pointX=point.pageX;this.pointY=point.pageY;this.distX+=deltaX;this.distY+=deltaY;absDistX=Math.abs(this.distX);absDistY=Math.abs(this.distY);if(timestamp-this.endTime>300&&(absDistX<10&&absDistY<10)){return}if(!this.directionLocked&&!this.options.freeScroll){if(absDistX>absDistY+this.options.directionLockThreshold){this.directionLocked="h"}else if(absDistY>=absDistX+this.options.directionLockThreshold){this.directionLocked="v"}else{this.directionLocked="n"}}if(this.directionLocked=="h"){if(this.options.eventPassthrough=="vertical"){e.preventDefault()}else if(this.options.eventPassthrough=="horizontal"){this.initiated=false;return}deltaY=0}else if(this.directionLocked=="v"){if(this.options.eventPassthrough=="horizontal"){e.preventDefault()}else if(this.options.eventPassthrough=="vertical"){this.initiated=false;return}deltaX=0}deltaX=this.hasHorizontalScroll?deltaX:0;deltaY=this.hasVerticalScroll?deltaY:0;newX=this.x+deltaX;newY=this.y+deltaY;if(newX>0||newX<this.maxScrollX){newX=this.options.bounce?this.x+deltaX/3:newX>0?0:this.maxScrollX}if(newY>0||newY<this.maxScrollY){newY=this.options.bounce?this.y+deltaY/3:newY>0?0:this.maxScrollY}this.directionX=deltaX>0?-1:deltaX<0?1:0;this.directionY=deltaY>0?-1:deltaY<0?1:0;if(!this.moved){this._execEvent("scrollStart")}this.moved=true;this._translate(newX,newY);if(timestamp-this.startTime>300){this.startTime=timestamp;this.startX=this.x;this.startY=this.y}},_end:function(e){if(!this.enabled||utils.eventType[e.type]!==this.initiated){return}if(this.options.preventDefault&&!utils.preventDefaultException(e.target,this.options.preventDefaultException)){e.preventDefault()}var point=e.changedTouches?e.changedTouches[0]:e,momentumX,momentumY,duration=utils.getTime()-this.startTime,newX=Math.round(this.x),newY=Math.round(this.y),distanceX=Math.abs(newX-this.startX),distanceY=Math.abs(newY-this.startY),time=0,easing="";this.scrollTo(newX,newY);this.isInTransition=0;this.initiated=0;this.endTime=utils.getTime();if(this.resetPosition(this.options.bounceTime)){return}if(!this.moved){if(this.options.tap){utils.tap(e,this.options.tap)}if(this.options.click){utils.click(e)}return}if(this._events.flick&&duration<200&&distanceX<100&&distanceY<100){this._execEvent("flick");return}if(this.options.momentum&&duration<300){momentumX=this.hasHorizontalScroll?utils.momentum(this.x,this.startX,duration,this.maxScrollX,this.options.bounce?this.wrapperWidth:0):{destination:newX,duration:0};momentumY=this.hasVerticalScroll?utils.momentum(this.y,this.startY,duration,this.maxScrollY,this.options.bounce?this.wrapperHeight:0):{destination:newY,duration:0};newX=momentumX.destination;newY=momentumY.destination;time=Math.max(momentumX.duration,momentumY.duration);this.isInTransition=1}if(newX!=this.x||newY!=this.y){if(newX>0||newX<this.maxScrollX||newY>0||newY<this.maxScrollY){easing=utils.ease.quadratic}this.scrollTo(newX,newY,time,easing);return}this._execEvent("scrollEnd")},_resize:function(){var that=this;clearTimeout(this.resizeTimeout);this.resizeTimeout=setTimeout(function(){that.refresh()},this.options.resizePolling)},resetPosition:function(time){var x=this.x,y=this.y;time=time||0;if(!this.hasHorizontalScroll||this.x>0){x=0}else if(this.x<this.maxScrollX){x=this.maxScrollX}if(!this.hasVerticalScroll||this.y>0){y=0}else if(this.y<this.maxScrollY){y=this.maxScrollY}if(x==this.x&&y==this.y){return false}this.scrollTo(x,y,time,this.options.bounceEasing);return true},disable:function(){this.enabled=false},enable:function(){this.enabled=true},refresh:function(){var rf=this.wrapper.offsetHeight;this.wrapperWidth=this.wrapper.clientWidth;this.wrapperHeight=this.wrapper.clientHeight;this.scrollerWidth=this.scroller.offsetWidth;this.scrollerHeight=this.scroller.offsetHeight;this.maxScrollX=this.wrapperWidth-this.scrollerWidth;this.maxScrollY=this.wrapperHeight-this.scrollerHeight;this.hasHorizontalScroll=this.options.scrollX&&this.maxScrollX<0;this.hasVerticalScroll=this.options.scrollY&&this.maxScrollY<0;if(!this.hasHorizontalScroll){this.maxScrollX=0;this.scrollerWidth=this.wrapperWidth}if(!this.hasVerticalScroll){this.maxScrollY=0;this.scrollerHeight=this.wrapperHeight}this.endTime=0;this.directionX=0;this.directionY=0;this.wrapperOffset=utils.offset(this.wrapper);this._execEvent("refresh");this.resetPosition()},on:function(type,fn){if(!this._events[type]){this._events[type]=[]}this._events[type].push(fn)},_execEvent:function(type){if(!this._events[type]){return}var i=0,l=this._events[type].length;if(!l){return}for(;i<l;i++){this._events[type][i].call(this)}},scrollBy:function(x,y,time,easing){x=this.x+x;y=this.y+y;time=time||0;this.scrollTo(x,y,time,easing)},scrollTo:function(x,y,time,easing){easing=easing||utils.ease.circular;if(!time||this.options.useTransition&&easing.style){this._transitionTimingFunction(easing.style);this._transitionTime(time);this._translate(x,y)}else{this._animate(x,y,time,easing.fn)}},scrollToElement:function(el,time,offsetX,offsetY,easing){el=el.nodeType?el:this.scroller.querySelector(el);if(!el){return}var pos=utils.offset(el);pos.left-=this.wrapperOffset.left;pos.top-=this.wrapperOffset.top;if(offsetX===true){offsetX=Math.round(el.offsetWidth/2-this.wrapper.offsetWidth/2)}if(offsetY===true){offsetY=Math.round(el.offsetHeight/2-this.wrapper.offsetHeight/2)}pos.left-=offsetX||0;pos.top-=offsetY||0;pos.left=pos.left>0?0:pos.left<this.maxScrollX?this.maxScrollX:pos.left;pos.top=pos.top>0?0:pos.top<this.maxScrollY?this.maxScrollY:pos.top;time=time===undefined||time===null||time==="auto"?Math.max(Math.abs(this.x-pos.left),Math.abs(this.y-pos.top)):time;this.scrollTo(pos.left,pos.top,time,easing)},_transitionTime:function(time){time=time||0;this.scrollerStyle[utils.style.transitionDuration]=time+"ms"},_transitionTimingFunction:function(easing){this.scrollerStyle[utils.style.transitionTimingFunction]=easing},_translate:function(x,y){if(this.options.useTransform){this.scrollerStyle[utils.style.transform]="translate("+x+"px,"+y+"px)"+this.translateZ}else{x=Math.round(x);y=Math.round(y);this.scrollerStyle.left=x+"px";this.scrollerStyle.top=y+"px"}this.x=x;this.y=y},_initEvents:function(remove){var eventType=remove?utils.removeEvent:utils.addEvent,target=this.options.bindToWrapper?this.wrapper:window;eventType(window,"orientationchange",this);eventType(window,"resize",this);if(this.options.click){eventType(this.wrapper,"click",this,true)}if(!this.options.disableMouse){eventType(this.wrapper,"mousedown",this);eventType(target,"mousemove",this);eventType(target,"mousecancel",this);eventType(target,"mouseup",this)}if(utils.hasPointer&&!this.options.disablePointer){eventType(this.wrapper,"MSPointerDown",this);eventType(target,"MSPointerMove",this);eventType(target,"MSPointerCancel",this);eventType(target,"MSPointerUp",this)}if(utils.hasTouch&&!this.options.disableTouch){eventType(this.wrapper,"touchstart",this);eventType(target,"touchmove",this);eventType(target,"touchcancel",this);eventType(target,"touchend",this)}eventType(this.scroller,"transitionend",this);eventType(this.scroller,"webkitTransitionEnd",this);eventType(this.scroller,"oTransitionEnd",this);eventType(this.scroller,"MSTransitionEnd",this)},getComputedPosition:function(){var matrix=window.getComputedStyle(this.scroller,null),x,y;if(this.options.useTransform){matrix=matrix[utils.style.transform].split(")")[0].split(", ");x=+(matrix[12]||matrix[4]);y=+(matrix[13]||matrix[5])}else{x=+matrix.left.replace(/[^-\d]/g,"");y=+matrix.top.replace(/[^-\d]/g,"")}return{x:x,y:y}},_animate:function(destX,destY,duration,easingFn){var that=this,startX=this.x,startY=this.y,startTime=utils.getTime(),destTime=startTime+duration;function step(){var now=utils.getTime(),newX,newY,easing;if(now>=destTime){that.isAnimating=false;that._translate(destX,destY);if(!that.resetPosition(that.options.bounceTime)){that._execEvent("scrollEnd")}return}now=(now-startTime)/duration;easing=easingFn(now);newX=(destX-startX)*easing+startX;newY=(destY-startY)*easing+startY;that._translate(newX,newY);if(that.isAnimating){rAF(step)}}this.isAnimating=true;step()},handleEvent:function(e){switch(e.type){case"touchstart":case"MSPointerDown":case"mousedown":this._start(e);break;case"touchmove":case"MSPointerMove":case"mousemove":this._move(e);break;case"touchend":case"MSPointerUp":case"mouseup":case"touchcancel":case"MSPointerCancel":case"mousecancel":this._end(e);break;case"orientationchange":case"resize":this._resize();break;case"transitionend":case"webkitTransitionEnd":case"oTransitionEnd":case"MSTransitionEnd":this._transitionEnd(e);break;case"DOMMouseScroll":case"mousewheel":this._wheel(e);break;case"keydown":this._key(e);break;case"click":if(!e._constructed){e.preventDefault();e.stopPropagation()}break}}};IScroll.ease=utils.ease;return IScroll}(window,document,Math);var MicroEvent=function(){};MicroEvent.prototype={on:function(event,fct){this._events=this._events||{};this._events[event]=this._events[event]||[];this._events[event].push(fct)},once:function(event,fct){var self=this;var wrapper=function(){self.off(event,wrapper);return fct.apply(null,arguments)};this.on(event,wrapper)},off:function(event,fct){this._events=this._events||{};if(event in this._events===false)return;this._events[event].splice(this._events[event].indexOf(fct),1)},emit:function(event){this._events=this._events||{};if(event in this._events===false)return;for(var i=0;i<this._events[event].length;i++){this._events[event][i].apply(this,Array.prototype.slice.call(arguments,1))}}};MicroEvent.mixin=function(destObject){var props=["on","once","off","emit"];for(var i=0;i<props.length;i++){if(typeof destObject==="function"){destObject.prototype[props[i]]=MicroEvent.prototype[props[i]]}else{destObject[props[i]]=MicroEvent.prototype[props[i]]}}};if(typeof module!=="undefined"&&"exports"in module){module.exports=MicroEvent}window.Modernizr=function(window,document,undefined){var version="2.6.2",Modernizr={},enableClasses=true,docElement=document.documentElement,mod="modernizr",modElem=document.createElement(mod),mStyle=modElem.style,inputElem,toString={}.toString,prefixes=" -webkit- -moz- -o- -ms- ".split(" "),omPrefixes="Webkit Moz O ms",cssomPrefixes=omPrefixes.split(" "),domPrefixes=omPrefixes.toLowerCase().split(" "),ns={svg:"http://www.w3.org/2000/svg"},tests={},inputs={},attrs={},classes=[],slice=classes.slice,featureName,injectElementWithStyles=function(rule,callback,nodes,testnames){var style,ret,node,docOverflow,div=document.createElement("div"),body=document.body,fakeBody=body||document.createElement("body");if(parseInt(nodes,10)){while(nodes--){node=document.createElement("div");node.id=testnames?testnames[nodes]:mod+(nodes+1);div.appendChild(node)}}style=["&#173;",'<style id="s',mod,'">',rule,"</style>"].join("");div.id=mod;(body?div:fakeBody).innerHTML+=style;fakeBody.appendChild(div);if(!body){fakeBody.style.background="";fakeBody.style.overflow="hidden";docOverflow=docElement.style.overflow;docElement.style.overflow="hidden";docElement.appendChild(fakeBody)}ret=callback(div,rule);if(!body){fakeBody.parentNode.removeChild(fakeBody);docElement.style.overflow=docOverflow}else{div.parentNode.removeChild(div)}return!!ret},_hasOwnProperty={}.hasOwnProperty,hasOwnProp;if(!is(_hasOwnProperty,"undefined")&&!is(_hasOwnProperty.call,"undefined")){hasOwnProp=function(object,property){return _hasOwnProperty.call(object,property)}}else{hasOwnProp=function(object,property){return property in object&&is(object.constructor.prototype[property],"undefined")}}if(!Function.prototype.bind){Function.prototype.bind=function bind(that){var target=this;if(typeof target!="function"){throw new TypeError}var args=slice.call(arguments,1),bound=function(){if(this instanceof bound){var F=function(){};F.prototype=target.prototype;var self=new F;var result=target.apply(self,args.concat(slice.call(arguments)));if(Object(result)===result){return result}return self}else{return target.apply(that,args.concat(slice.call(arguments)))}};return bound}}function setCss(str){mStyle.cssText=str}function setCssAll(str1,str2){return setCss(prefixes.join(str1+";")+(str2||""))}function is(obj,type){return typeof obj===type}function contains(str,substr){return!!~(""+str).indexOf(substr)}function testProps(props,prefixed){for(var i in props){var prop=props[i];if(!contains(prop,"-")&&mStyle[prop]!==undefined){return prefixed=="pfx"?prop:true}}return false}function testDOMProps(props,obj,elem){for(var i in props){var item=obj[props[i]];if(item!==undefined){if(elem===false)return props[i];if(is(item,"function")){return item.bind(elem||obj)}return item}}return false}function testPropsAll(prop,prefixed,elem){var ucProp=prop.charAt(0).toUpperCase()+prop.slice(1),props=(prop+" "+cssomPrefixes.join(ucProp+" ")+ucProp).split(" ");if(is(prefixed,"string")||is(prefixed,"undefined")){return testProps(props,prefixed)}else{props=(prop+" "+domPrefixes.join(ucProp+" ")+ucProp).split(" ");return testDOMProps(props,prefixed,elem)}}tests["canvas"]=function(){var elem=document.createElement("canvas");return!!(elem.getContext&&elem.getContext("2d"))};tests["borderradius"]=function(){return testPropsAll("borderRadius")};tests["boxshadow"]=function(){return testPropsAll("boxShadow")};tests["cssanimations"]=function(){return testPropsAll("animationName")};tests["csstransforms"]=function(){return!!testPropsAll("transform")};tests["csstransforms3d"]=function(){var ret=!!testPropsAll("perspective");if(ret&&"webkitPerspective"in docElement.style){injectElementWithStyles("@media (transform-3d),(-webkit-transform-3d){#modernizr{left:9px;position:absolute;height:3px;}}",function(node,rule){ret=node.offsetLeft===9&&node.offsetHeight===3})}return ret};tests["csstransitions"]=function(){return testPropsAll("transition")};tests["svg"]=function(){return!!document.createElementNS&&!!document.createElementNS(ns.svg,"svg").createSVGRect};for(var feature in tests){if(hasOwnProp(tests,feature)){featureName=feature.toLowerCase();Modernizr[featureName]=tests[feature]();classes.push((Modernizr[featureName]?"":"no-")+featureName)}}Modernizr.addTest=function(feature,test){if(typeof feature=="object"){for(var key in feature){if(hasOwnProp(feature,key)){Modernizr.addTest(key,feature[key])}}}else{feature=feature.toLowerCase();if(Modernizr[feature]!==undefined){return Modernizr}test=typeof test=="function"?test():test;if(typeof enableClasses!=="undefined"&&enableClasses){docElement.className+=" "+(test?"":"no-")+feature}Modernizr[feature]=test}return Modernizr};setCss("");modElem=inputElem=null;(function(window,document){var options=window.html5||{};var reSkip=/^<|^(?:button|map|select|textarea|object|iframe|option|optgroup)$/i;var saveClones=/^(?:a|b|code|div|fieldset|h1|h2|h3|h4|h5|h6|i|label|li|ol|p|q|span|strong|style|table|tbody|td|th|tr|ul)$/i;var supportsHtml5Styles;var expando="_html5shiv";var expanID=0;var expandoData={};var supportsUnknownElements;(function(){try{var a=document.createElement("a");a.innerHTML="<xyz></xyz>";supportsHtml5Styles="hidden"in a;supportsUnknownElements=a.childNodes.length==1||function(){document.createElement("a");var frag=document.createDocumentFragment();return typeof frag.cloneNode=="undefined"||typeof frag.createDocumentFragment=="undefined"||typeof frag.createElement=="undefined"}()}catch(e){supportsHtml5Styles=true;supportsUnknownElements=true}})();function addStyleSheet(ownerDocument,cssText){var p=ownerDocument.createElement("p"),parent=ownerDocument.getElementsByTagName("head")[0]||ownerDocument.documentElement;p.innerHTML="x<style>"+cssText+"</style>";return parent.insertBefore(p.lastChild,parent.firstChild)}function getElements(){var elements=html5.elements;return typeof elements=="string"?elements.split(" "):elements}function getExpandoData(ownerDocument){var data=expandoData[ownerDocument[expando]];if(!data){data={};expanID++;ownerDocument[expando]=expanID;expandoData[expanID]=data}return data}function createElement(nodeName,ownerDocument,data){if(!ownerDocument){ownerDocument=document}if(supportsUnknownElements){return ownerDocument.createElement(nodeName)}if(!data){data=getExpandoData(ownerDocument)}var node;if(data.cache[nodeName]){node=data.cache[nodeName].cloneNode()}else if(saveClones.test(nodeName)){node=(data.cache[nodeName]=data.createElem(nodeName)).cloneNode()}else{node=data.createElem(nodeName)}return node.canHaveChildren&&!reSkip.test(nodeName)?data.frag.appendChild(node):node}function createDocumentFragment(ownerDocument,data){if(!ownerDocument){ownerDocument=document}if(supportsUnknownElements){return ownerDocument.createDocumentFragment()}data=data||getExpandoData(ownerDocument);var clone=data.frag.cloneNode(),i=0,elems=getElements(),l=elems.length;for(;i<l;i++){clone.createElement(elems[i])}return clone}function shivMethods(ownerDocument,data){if(!data.cache){data.cache={};data.createElem=ownerDocument.createElement;data.createFrag=ownerDocument.createDocumentFragment;data.frag=data.createFrag()}ownerDocument.createElement=function(nodeName){if(!html5.shivMethods){return data.createElem(nodeName)}return createElement(nodeName,ownerDocument,data)};ownerDocument.createDocumentFragment=Function("h,f","return function(){"+"var n=f.cloneNode(),c=n.createElement;"+"h.shivMethods&&("+getElements().join().replace(/\w+/g,function(nodeName){data.createElem(nodeName);data.frag.createElement(nodeName);return'c("'+nodeName+'")'})+");return n}")(html5,data.frag)}function shivDocument(ownerDocument){if(!ownerDocument){ownerDocument=document}var data=getExpandoData(ownerDocument);if(html5.shivCSS&&!supportsHtml5Styles&&!data.hasCSS){data.hasCSS=!!addStyleSheet(ownerDocument,"article,aside,figcaption,figure,footer,header,hgroup,nav,section{display:block}"+"mark{background:#FF0;color:#000}")}if(!supportsUnknownElements){shivMethods(ownerDocument,data)}return ownerDocument}var html5={elements:options.elements||"abbr article aside audio bdi canvas data datalist details figcaption figure footer header hgroup mark meter nav output progress section summary time video",shivCSS:options.shivCSS!==false,supportsUnknownElements:supportsUnknownElements,shivMethods:options.shivMethods!==false,type:"default",shivDocument:shivDocument,createElement:createElement,createDocumentFragment:createDocumentFragment};window.html5=html5;shivDocument(document)})(this,document);Modernizr._version=version;Modernizr._prefixes=prefixes;Modernizr._domPrefixes=domPrefixes;Modernizr._cssomPrefixes=cssomPrefixes;Modernizr.testProp=function(prop){return testProps([prop])};Modernizr.testAllProps=testPropsAll;Modernizr.testStyles=injectElementWithStyles;docElement.className=docElement.className.replace(/(^|\s)no-js(\s|$)/,"$1$2")+(enableClasses?" js "+classes.join(" "):"");return Modernizr}(this,this.document);(function(a,b,c){function d(a){return"[object Function]"==o.call(a)}function e(a){return"string"==typeof a}function f(){}function g(a){return!a||"loaded"==a||"complete"==a||"uninitialized"==a}function h(){var a=p.shift();q=1,a?a.t?m(function(){("c"==a.t?B.injectCss:B.injectJs)(a.s,0,a.a,a.x,a.e,1)},0):(a(),h()):q=0}function i(a,c,d,e,f,i,j){function k(b){if(!o&&g(l.readyState)&&(u.r=o=1,!q&&h(),l.onload=l.onreadystatechange=null,b)){"img"!=a&&m(function(){t.removeChild(l)},50);for(var d in y[c])y[c].hasOwnProperty(d)&&y[c][d].onload()}}var j=j||B.errorTimeout,l=b.createElement(a),o=0,r=0,u={t:d,s:c,e:f,a:i,x:j};1===y[c]&&(r=1,y[c]=[]),"object"==a?l.data=c:(l.src=c,l.type=a),l.width=l.height="0",l.onerror=l.onload=l.onreadystatechange=function(){k.call(this,r)},p.splice(e,0,u),"img"!=a&&(r||2===y[c]?(t.insertBefore(l,s?null:n),m(k,j)):y[c].push(l))}function j(a,b,c,d,f){return q=0,b=b||"j",e(a)?i("c"==b?v:u,a,b,this.i++,c,d,f):(p.splice(this.i++,0,a),1==p.length&&h()),this}function k(){var a=B;return a.loader={load:j,i:0},a}var l=b.documentElement,m=a.setTimeout,n=b.getElementsByTagName("script")[0],o={}.toString,p=[],q=0,r="MozAppearance"in l.style,s=r&&!!b.createRange().compareNode,t=s?l:n.parentNode,l=a.opera&&"[object Opera]"==o.call(a.opera),l=!!b.attachEvent&&!l,u=r?"object":l?"script":"img",v=l?"script":u,w=Array.isArray||function(a){return"[object Array]"==o.call(a)},x=[],y={},z={timeout:function(a,b){return b.length&&(a.timeout=b[0]),a}},A,B;B=function(a){function b(a){var a=a.split("!"),b=x.length,c=a.pop(),d=a.length,c={url:c,origUrl:c,prefixes:a},e,f,g;for(f=0;f<d;f++)g=a[f].split("="),(e=z[g.shift()])&&(c=e(c,g));for(f=0;f<b;f++)c=x[f](c);return c}function g(a,e,f,g,h){var i=b(a),j=i.autoCallback;i.url.split(".").pop().split("?").shift(),i.bypass||(e&&(e=d(e)?e:e[a]||e[g]||e[a.split("/").pop().split("?")[0]]),i.instead?i.instead(a,e,f,g,h):(y[i.url]?i.noexec=!0:y[i.url]=1,f.load(i.url,i.forceCSS||!i.forceJS&&"css"==i.url.split(".").pop().split("?").shift()?"c":c,i.noexec,i.attrs,i.timeout),(d(e)||d(j))&&f.load(function(){k(),e&&e(i.origUrl,h,g),j&&j(i.origUrl,h,g),y[i.url]=2})))}function h(a,b){function c(a,c){if(a){if(e(a))c||(j=function(){var a=[].slice.call(arguments);k.apply(this,a),l()}),g(a,j,b,0,h);else if(Object(a)===a)for(n in m=function(){var b=0,c;for(c in a)a.hasOwnProperty(c)&&b++;return b}(),a)a.hasOwnProperty(n)&&(!c&&!--m&&(d(j)?j=function(){var a=[].slice.call(arguments);k.apply(this,a),l()}:j[n]=function(a){return function(){var b=[].slice.call(arguments);a&&a.apply(this,b),l()}}(k[n])),g(a[n],j,b,n,h))}else!c&&l()}var h=!!a.test,i=a.load||a.both,j=a.callback||f,k=j,l=a.complete||f,m,n;c(h?a.yep:a.nope,!!i),i&&c(i)}var i,j,l=this.yepnope.loader;if(e(a))g(a,0,l,0);else if(w(a))for(i=0;i<a.length;i++)j=a[i],e(j)?g(j,0,l,0):w(j)?B(j):Object(j)===j&&h(j,l);else Object(a)===a&&h(a,l)},B.addPrefix=function(a,b){z[a]=b},B.addFilter=function(a){x.push(a)},B.errorTimeout=1e4,null==b.readyState&&b.addEventListener&&(b.readyState="loading",b.addEventListener("DOMContentLoaded",A=function(){b.removeEventListener("DOMContentLoaded",A,0),b.readyState="complete"},0)),a.yepnope=k(),a.yepnope.executeStack=h,a.yepnope.injectJs=function(a,c,d,e,i,j){var k=b.createElement("script"),l,o,e=e||B.errorTimeout;k.src=a;for(o in d)k.setAttribute(o,d[o]);c=j?h:c||f,k.onreadystatechange=k.onload=function(){!l&&g(k.readyState)&&(l=1,c(),k.onload=k.onreadystatechange=null)},m(function(){l||(l=1,c(1))},e),i?k.onload():n.parentNode.insertBefore(k,n)},a.yepnope.injectCss=function(a,c,d,e,g,i){var e=b.createElement("link"),j,c=i?h:c||f;e.href=a,e.rel="stylesheet",e.type="text/css";for(j in d)e.setAttribute(j,d[j]);g||(n.parentNode.insertBefore(e,n),m(c,0))}})(this,document);Modernizr.load=function(){yepnope.apply(window,[].slice.call(arguments,0))};(function(global,undefined){"use strict";if(global.setImmediate){return}var nextHandle=1;var tasksByHandle={};var currentlyRunningATask=false;var doc=global.document;var setImmediate;function addFromSetImmediateArguments(args){tasksByHandle[nextHandle]=partiallyApplied.apply(undefined,args);return nextHandle++}function partiallyApplied(handler){var args=[].slice.call(arguments,1);return function(){if(typeof handler==="function"){handler.apply(undefined,args)}else{new Function(""+handler)()}}}function runIfPresent(handle){if(currentlyRunningATask){setTimeout(partiallyApplied(runIfPresent,handle),0)}else{var task=tasksByHandle[handle];if(task){currentlyRunningATask=true;try{task()}finally{clearImmediate(handle);currentlyRunningATask=false}}}}function clearImmediate(handle){delete tasksByHandle[handle]}function installNextTickImplementation(){setImmediate=function(){var handle=addFromSetImmediateArguments(arguments);process.nextTick(partiallyApplied(runIfPresent,handle));return handle}}function canUsePostMessage(){if(global.postMessage&&!global.importScripts){var postMessageIsAsynchronous=true;var oldOnMessage=global.onmessage;global.onmessage=function(){postMessageIsAsynchronous=false};global.postMessage("","*");global.onmessage=oldOnMessage;return postMessageIsAsynchronous}}function installPostMessageImplementation(){var messagePrefix="setImmediate$"+Math.random()+"$";var onGlobalMessage=function(event){if(event.source===global&&typeof event.data==="string"&&event.data.indexOf(messagePrefix)===0){runIfPresent(+event.data.slice(messagePrefix.length))}};if(global.addEventListener){global.addEventListener("message",onGlobalMessage,false)}else{global.attachEvent("onmessage",onGlobalMessage)}setImmediate=function(){var handle=addFromSetImmediateArguments(arguments);global.postMessage(messagePrefix+handle,"*");return handle}}function installMessageChannelImplementation(){var channel=new MessageChannel;channel.port1.onmessage=function(event){var handle=event.data;runIfPresent(handle)};setImmediate=function(){var handle=addFromSetImmediateArguments(arguments);channel.port2.postMessage(handle);return handle}}function installReadyStateChangeImplementation(){var html=doc.documentElement;setImmediate=function(){var handle=addFromSetImmediateArguments(arguments);var script=doc.createElement("script");script.onreadystatechange=function(){runIfPresent(handle);script.onreadystatechange=null;html.removeChild(script);script=null};html.appendChild(script);return handle}}function installSetTimeoutImplementation(){setImmediate=function(){var handle=addFromSetImmediateArguments(arguments);setTimeout(partiallyApplied(runIfPresent,handle),0);return handle}}var attachTo=Object.getPrototypeOf&&Object.getPrototypeOf(global);attachTo=attachTo&&attachTo.setTimeout?attachTo:global;if({}.toString.call(global.process)==="[object process]"){installNextTickImplementation()}else if(canUsePostMessage()){installPostMessageImplementation()}else if(global.MessageChannel){installMessageChannelImplementation()}else if(doc&&"onreadystatechange"in doc.createElement("script")){installReadyStateChangeImplementation()}else{installSetTimeoutImplementation()}attachTo.setImmediate=setImmediate;attachTo.clearImmediate=clearImmediate})(new Function("return this")());(function(){function Viewport(){this.PRE_IOS7_VIEWPORT="initial-scale=1, maximum-scale=1, user-scalable=no";this.IOS7_VIEWPORT="width=device-width, height=device-height, initial-scale=1, maximum-scale=1, user-scalable=no";this.DEFAULT_VIEWPORT="initial-scale=1, maximum-scale=1, user-scalable=no";this.ensureViewportElement();this.platform={};this.platform.name=this.getPlatformName();this.platform.version=this.getPlatformVersion();return this}Viewport.prototype.ensureViewportElement=function(){this.viewportElement=document.querySelector("meta[name=viewport]");if(!this.viewportElement){this.viewportElement=document.createElement("meta");this.viewportElement.name="viewport";document.head.appendChild(this.viewportElement)}},Viewport.prototype.setup=function(){if(!this.viewportElement){return}if(this.viewportElement.getAttribute("data-no-adjust")=="true"){return}if(this.platform.name=="ios"){if(this.platform.version>=7&&isWebView()){this.viewportElement.setAttribute("content",this.IOS7_VIEWPORT)}else{this.viewportElement.setAttribute("content",this.PRE_IOS7_VIEWPORT)}}else{this.viewportElement.setAttribute("content",this.DEFAULT_VIEWPORT)}function isWebView(){return!!(window.cordova||window.phonegap||window.PhoneGap)}};Viewport.prototype.getPlatformName=function(){if(navigator.userAgent.match(/Android/i)){return"android"}if(navigator.userAgent.match(/iPhone|iPad|iPod/i)){return"ios"}return undefined};Viewport.prototype.getPlatformVersion=function(){var start=window.navigator.userAgent.indexOf("OS ");return window.Number(window.navigator.userAgent.substr(start+3,3).replace("_","."))};window.Viewport=Viewport})();(function(module){try{app=angular.module("templates-main")}catch(err){app=angular.module("templates-main",[])}app.run(["$templateCache",function($templateCache){"use strict";$templateCache.put("templates/back_button.tpl",'<span class="toolbar-button--quiet {{modifierTemplater(\'toolbar-button--quiet--*\')}}" ng-click="$root.ons.findParentComponentUntil(\'ons-navigator\', $event).popPage()" style="height: 44px; line-height: 0; padding: 0; position: relative;">\n'+'  <i class="fa fa-angle-left ons-back-button__icon" style="vertical-align: top; line-height: 44px; font-size: 36px; padding-left: 8px; padding-right: 4px; height: 44px; width: 14px;"></i><span style="vertical-align: top; display: inline-block; line-height: 44px; height: 44px;" class="back-button__label"></span>\n'+"</span>\n"+"")}])})();(function(module){try{app=angular.module("templates-main")}catch(err){app=angular.module("templates-main",[])}app.run(["$templateCache",function($templateCache){"use strict";$templateCache.put("templates/button.tpl",'<span class="label ons-button-inner"></span>\n'+"<span class=\"spinner button__spinner {{modifierTemplater('button--*__spinner')}}\"></span>\n"+"")}])})();(function(module){try{app=angular.module("templates-main")
}catch(err){app=angular.module("templates-main",[])}app.run(["$templateCache",function($templateCache){"use strict";$templateCache.put("templates/icon.tpl",'<i class="fa fa-{{icon}} fa-{{spin}} fa-{{fixedWidth}} fa-rotate-{{rotate}} fa-flip-{{flip}}" ng-class="sizeClass" ng-style="style"></i>\n'+"")}])})();(function(module){try{app=angular.module("templates-main")}catch(err){app=angular.module("templates-main",[])}app.run(["$templateCache",function($templateCache){"use strict";$templateCache.put("templates/row.tpl",'<div class="row row-{{align}} ons-row-inner"></div>\n'+"")}])})();(function(module){try{app=angular.module("templates-main")}catch(err){app=angular.module("templates-main",[])}app.run(["$templateCache",function($templateCache){"use strict";$templateCache.put("templates/screen.tpl",'<div class="ons-screen"></div>\n'+"")}])})();(function(module){try{app=angular.module("templates-main")}catch(err){app=angular.module("templates-main",[])}app.run(["$templateCache",function($templateCache){"use strict";$templateCache.put("templates/sliding_menu.tpl",'<div class="onsen-sliding-menu__behind ons-sliding-menu-inner"></div>\n'+'<div class="onsen-sliding-menu__above ons-sliding-menu-inner"></div>\n'+"")}])})();(function(module){try{app=angular.module("templates-main")}catch(err){app=angular.module("templates-main",[])}app.run(["$templateCache",function($templateCache){"use strict";$templateCache.put("templates/split_view.tpl",'<div class="onsen-split-view__secondary full-screen ons-split-view-inner"></div>\n'+'<div class="onsen-split-view__main full-screen ons-split-view-inner"></div>\n'+"")}])})();(function(module){try{app=angular.module("templates-main")}catch(err){app=angular.module("templates-main",[])}app.run(["$templateCache",function($templateCache){"use strict";$templateCache.put("templates/switch.tpl","<label class=\"switch {{modifierTemplater('switch--*')}}\">\n"+'  <input type="checkbox" class="switch__input {{modifierTemplater(\'switch--*__input\')}}" ng-model="model">\n'+"  <div class=\"switch__toggle {{modifierTemplater('switch--*__toggle')}}\"></div>\n"+"</label>\n"+"")}])})();(function(module){try{app=angular.module("templates-main")}catch(err){app=angular.module("templates-main",[])}app.run(["$templateCache",function($templateCache){"use strict";$templateCache.put("templates/tab_bar.tpl",'<div style="margin-bottom: {{tabbarHeight}}" class="ons-tab-bar__content"></div>\n'+'<div ng-hide="hideTabs" class="tab-bar ons-tab-bar__footer {{modifierTemplater(\'tab-bar--*\')}} ons-tabbar-inner"></div>\n'+"")}])})();(function(module){try{app=angular.module("templates-main")}catch(err){app=angular.module("templates-main",[])}app.run(["$templateCache",function($templateCache){"use strict";$templateCache.put("templates/tab_bar_item.tpl","<label class=\"tab-bar__item {{tabbarModifierTemplater('tab-bar--*__item')}} {{modifierTemplater('tab-bar__item--*')}}\">\n"+'  <input type="radio" name="tab-bar-{{tabbarId}}" style="display: none">\n'+"  <button class=\"tab-bar__button {{tabbarModifierTemplater('tab-bar--*__button')}} {{modifierTemplater('tab-bar__button--*')}}\" ng-click=\"tryToChange()\">\n"+'    <div ng-if="icon != undefined" class="tab-bar__icon"><ons-icon icon="{{tabIcon}}" style="font-size: 28px; line-height: 34px; vertical-align: top;"></ons-icon></div>\n'+'    <div ng-if="label" class="tab-bar__label">{{label}}</div>\n'+"  </button>\n"+"</label>\n"+"")}])})();(function(module){try{app=angular.module("templates-main")}catch(err){app=angular.module("templates-main",[])}app.run(["$templateCache",function($templateCache){"use strict";$templateCache.put("templates/toolbar_button.tpl","<span class=\"toolbar-button {{modifierTemplater('toolbar-button--quiet--*')}} navigation-bar__line-height\" ng-transclude></span>\n"+"")}])})();window.DoorLock=function(){var DoorLock=function(options){options=options||{};this._lockList=[];this._waitList=[];this._log=options.log||function(){}};DoorLock.generateId=function(){var i=0;return function(){return i++}}();DoorLock.prototype={lock:function(){var self=this;var unlock=function(){self._unlock(unlock)};unlock.id=DoorLock.generateId();this._lockList.push(unlock);this._log("lock: "+unlock.id);return unlock},_unlock:function(fn){var index=this._lockList.indexOf(fn);if(index===-1){throw new Error("This function is not registered in the lock list.")}this._lockList.splice(index,1);this._log("unlock: "+fn.id);this._tryToFreeWaitList()},_tryToFreeWaitList:function(){while(!this.isLocked()&&this._waitList.length>0){this._waitList.shift()()}},waitUnlock:function(callback){if(!(callback instanceof Function)){throw new Error("The callback param must be a function.")}if(this.isLocked()){this._waitList.push(callback)}else{callback()}},isLocked:function(){return this._lockList.length>0}};return DoorLock}();(function(){"use strict";var module=angular.module("onsen",["templates-main"]);angular.module("onsen.directives",["onsen"]);var ons=window.ons={_readyLock:new DoorLock,_onsenService:null,_unlockersDict:{},bootstrap:function(deps){deps=["onsen"].concat(angular.isArray(deps)?deps:[]);var doc=window.document;if(doc.readyState=="loading"||doc.readyState=="uninitialized"){doc.addEventListener("DOMContentLoaded",function(){angular.bootstrap(doc.documentElement,deps)},false)}else if(doc.documentElement){angular.bootstrap(doc.documentElement,deps)}else{throw new Error("Invalid state")}},findParentComponentUntil:function(name,dom){var element;if(dom instanceof HTMLElement){element=angular.element(dom)}else if(dom instanceof angular.element){element=dom}else if(dom.target){element=angular.element(dom.target)}return element.inheritedData(name)},setDefaultDeviceBackButtonListener:function(listener){this._getOnsenService().getDefaultDeviceBackButtonHandler().setListener(listener)},disableDeviceBackButtonHandler:function(){this._getOnsenService().DeviceBackButtonHandler.disable()},enableDeviceBackButtonHandler:function(){this._getOnsenService().DeviceBackButtonHandler.enable()},findComponent:function(selector,dom){var target=(dom?dom:document).querySelector(selector);return target?angular.element(target).data(target.nodeName.toLowerCase())||null:null},isReady:function(){return!ons._readyLock.isLocked()},compile:function(dom){if(!ons.$compile){throw new Error("ons.$compile() is not ready. Wait for initialization with ons.ready().")}if(!(dom instanceof HTMLElement)){throw new Error("First argument must be an instance of HTMLElement.")}var scope=angular.element(dom).scope();if(!scope){throw new Error("AngularJS Scope is null. Argument DOM element must be attached in DOM document.")}ons.$compile(dom)(scope)},_getOnsenService:function(){if(!this._onsenService){throw new Error("$onsen is not loaded, wait for ons.ready().")}return this._onsenService},ready:function(callback){if(callback instanceof Function){if(ons.isReady()){callback()}else{ons._readyLock.waitUnlock(callback)}}else if(angular.isArray(callback)&&arguments[1]instanceof Function){var dependencies=callback;callback=arguments[1];ons.ready(function(){var $onsen=ons._getOnsenService();$onsen.waitForVariables(dependencies,callback)})}},isWebView:function(){if(document.readyState==="loading"||document.readyState=="uninitialized"){throw new Error("isWebView() method is available after dom contents loaded.")}return!!(window.cordova||window.phonegap||window.PhoneGap)}};waitDeviceReady();waitOnsenUILoad();init();function waitDeviceReady(){var unlockDeviceReady=ons._readyLock.lock();window.addEventListener("DOMContentLoaded",function(){if(ons.isWebView()){window.document.addEventListener("deviceready",unlockDeviceReady,false)}else{unlockDeviceReady()}},false)}function waitOnsenUILoad(){var unlockOnsenUI=ons._readyLock.lock();module.run(["$compile","$rootScope","$onsen",function($compile,$rootScope,$onsen){if(document.readyState==="loading"||document.readyState=="uninitialized"){window.addEventListener("DOMContentLoaded",function(){document.body.appendChild(document.createElement("ons-dummy-for-init"))})}else if(document.body){document.body.appendChild(document.createElement("ons-dummy-for-init"))}else{throw new Error("Invalid initialization state.")}$rootScope.$on("$ons-ready",unlockOnsenUI)}])}function init(){module.run(["$compile","$rootScope","$onsen",function($compile,$rootScope,$onsen){ons._onsenService=$onsen;$rootScope.ons=window.ons;$rootScope.console=window.console;$rootScope.alert=window.alert;ons.$compile=$compile}])}})();(function(){"use strict;";var module=angular.module("onsen");module.factory("FadeTransitionAnimator",["NavigatorTransitionAnimator",function(NavigatorTransitionAnimator){var FadeTransitionAnimator=NavigatorTransitionAnimator.extend({push:function(enterPage,leavePage,callback){animit.runAll(animit(enterPage.getPageView().getContentElement()).queue({css:{transform:"translate3D(0, 0, 0)",opacity:0},duration:0}).queue({css:{transform:"translate3D(0, 0, 0)",opacity:1},duration:.4,timing:"linear"}).resetStyle().queue(function(done){callback();done()}),animit(enterPage.getPageView().getToolbarElement()).queue({css:{transform:"translate3D(0, 0, 0)",opacity:0},duration:0}).queue({css:{transform:"translate3D(0, 0, 0)",opacity:1},duration:.4,timing:"linear"}).resetStyle())},pop:function(enterPage,leavePage,callback){animit.runAll(animit(leavePage.getPageView().getContentElement()).queue({css:{transform:"translate3D(0, 0, 0)",opacity:1},duration:0}).queue({css:{transform:"translate3D(0, 0, 0)",opacity:0},duration:.4,timing:"linear"}).queue(function(done){callback();done()}),animit(leavePage.getPageView().getToolbarElement()).queue({css:{transform:"translate3D(0, 0, 0)",opacity:1},duration:0}).queue({css:{transform:"translate3D(0, 0, 0)",opacity:0},duration:.4,timing:"linear"}))}});return FadeTransitionAnimator}])})();(function(){"use strict;";var module=angular.module("onsen");module.factory("IOSSlideTransitionAnimator",["NavigatorTransitionAnimator","PageView",function(NavigatorTransitionAnimator,PageView){var IOSSlideTransitionAnimator=NavigatorTransitionAnimator.extend({backgroundMask:angular.element('<div style="position: absolute; width: 100%;'+'height: 100%; background-color: black; opacity: 0;"></div>'),_decompose:function(page){var elements=[];var left=page.getPageView().getToolbarLeftItemsElement();var right=page.getPageView().getToolbarRightItemsElement();var other=[].concat(left.children.length===0?left:excludeBackButtonLabel(left.children)).concat(right.children.length===0?right:excludeBackButtonLabel(right.children));var pageLabels=[page.getPageView().getToolbarCenterItemsElement(),page.getPageView().getToolbarBackButtonLabelElement()];return{pageLabels:pageLabels,other:other,content:page.getPageView().getContentElement(),toolbar:page.getPageView().getToolbarElement(),bottomToolbar:page.getPageView().getBottomToolbarElement()};function excludeBackButtonLabel(elements){var result=[];for(var i=0;i<elements.length;i++){if(elements[i].nodeName.toLowerCase()==="ons-back-button"){result.push(elements[i].querySelector(".ons-back-button__icon"))}else{result.push(elements[i])}}return result}},push:function(enterPage,leavePage,callback){var mask=this.backgroundMask.remove();leavePage.element[0].parentNode.insertBefore(mask[0],leavePage.element[0].nextSibling);var enterPageDecomposition=this._decompose(enterPage);var leavePageDecomposition=this._decompose(leavePage);var delta=function(){var rect=leavePage.element[0].getBoundingClientRect();return Math.round((rect.right-rect.left)/2*.6)}();var maskClear=animit(mask[0]).queue({opacity:0,transform:"translate3d(0, 0, 0)"}).queue({opacity:.1},{duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).resetStyle().queue(function(done){mask.remove();done()});var bothPageHasToolbar=enterPage.getPageView().hasToolbarElement()&&leavePage.getPageView().hasToolbarElement();var isToolbarNothing=!enterPage.getPageView().hasToolbarElement()&&!leavePage.getPageView().hasToolbarElement();if(bothPageHasToolbar){animit.runAll(maskClear,animit([enterPageDecomposition.content,enterPageDecomposition.bottomToolbar]).queue({css:{transform:"translate3D(100%, 0px, 0px)"},duration:0}).queue({css:{transform:"translate3D(0px, 0px, 0px)"},duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).resetStyle(),animit(enterPageDecomposition.toolbar).queue({css:{background:"none",backgroundColor:"rgba(0, 0, 0, 0)",borderColor:"rgba(0, 0, 0, 0)"},duration:0}).wait(.3).resetStyle({duration:.1,transition:"background-color 0.1s linear, "+"border-color 0.1s linear"}),animit(enterPageDecomposition.pageLabels).queue({css:{transform:"translate3d("+delta+"px, 0, 0)",opacity:0},duration:0}).queue({css:{transform:"translate3d(0, 0, 0)",opacity:1},duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).resetStyle(),animit(enterPageDecomposition.other).queue({css:{opacity:0},duration:0}).queue({css:{opacity:1},duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).resetStyle(),animit([leavePageDecomposition.content,leavePageDecomposition.bottomToolbar]).queue({css:{transform:"translate3D(0, 0, 0)"},duration:0}).queue({css:{transform:"translate3D(-25%, 0px, 0px)"},duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).resetStyle().queue(function(done){callback();done()}),animit(leavePageDecomposition.pageLabels).queue({css:{transform:"translate3d(0, 0, 0)",opacity:1},duration:0}).queue({css:{transform:"translate3d(-"+delta+"px, 0, 0)",opacity:0},duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).resetStyle(),animit(leavePageDecomposition.other).queue({css:{opacity:1},duration:0}).queue({css:{opacity:0},duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).resetStyle())}else{animit.runAll(maskClear,animit(enterPage.element[0]).queue({css:{transform:"translate3D(100%, 0px, 0px)"},duration:0}).queue({css:{transform:"translate3D(0px, 0px, 0px)"},duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).resetStyle(),animit(leavePage.element[0]).queue({css:{transform:"translate3D(0, 0, 0)"},duration:0}).queue({css:{transform:"translate3D(-25%, 0px, 0px)"},duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).resetStyle().queue(function(done){callback();done()}))}},pop:function(enterPage,leavePage,done){var mask=this.backgroundMask.remove();enterPage.element[0].parentNode.insertBefore(mask[0],enterPage.element[0].nextSibling);var enterPageDecomposition=this._decompose(enterPage);var leavePageDecomposition=this._decompose(leavePage);var delta=function(){var rect=leavePage.element[0].getBoundingClientRect();return Math.round((rect.right-rect.left)/2*.6)}();var maskClear=animit(mask[0]).queue({opacity:.1,transform:"translate3d(0, 0, 0)"}).queue({opacity:0},{duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).resetStyle().queue(function(done){mask.remove();done()});var bothPageHasToolbar=enterPage.getPageView().hasToolbarElement()&&leavePage.getPageView().hasToolbarElement();var isToolbarNothing=!enterPage.getPageView().hasToolbarElement()&&!leavePage.getPageView().hasToolbarElement();if(bothPageHasToolbar||isToolbarNothing){animit.runAll(maskClear,animit([enterPageDecomposition.content,enterPageDecomposition.bottomToolbar]).queue({css:{transform:"translate3D(-25%, 0px, 0px)",opacity:.9},duration:0}).queue({css:{transform:"translate3D(0px, 0px, 0px)",opacity:1},duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).resetStyle(),animit(enterPageDecomposition.pageLabels).queue({css:{transform:"translate3d(-"+delta+"px, 0, 0)",opacity:0},duration:0}).queue({css:{transform:"translate3d(0, 0, 0)",opacity:1},duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).resetStyle(),animit(enterPageDecomposition.toolbar).queue({css:{transform:"translate3d(0, 0, 0)",opacity:1},duration:0}).queue({css:{transform:"translate3d(0, 0, 0)",opacity:1},duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).resetStyle(),animit(enterPageDecomposition.other).queue({css:{opacity:0},duration:0}).queue({css:{opacity:1},duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).resetStyle(),animit([leavePageDecomposition.content,leavePageDecomposition.bottomToolbar]).queue({css:{transform:"translate3D(0px, 0px, 0px)"},duration:0}).queue({css:{transform:"translate3D(100%, 0px, 0px)"},duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).wait(0).queue(function(finish){done();finish()}),animit(leavePageDecomposition.other).queue({css:{transform:"translate3d(0, 0, 0)",opacity:1},duration:0}).queue({css:{transform:"translate3d(0, 0, 0)",opacity:0},duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}),animit(leavePageDecomposition.toolbar).queue({css:{background:"none",backgroundColor:"rgba(0, 0, 0, 0)",borderColor:"rgba(0, 0, 0, 0)"},duration:0}),animit(leavePageDecomposition.pageLabels).queue({css:{transform:"translate3d(0, 0, 0)",opacity:1},duration:0}).queue({css:{transform:"translate3d("+delta+"px, 0, 0)",opacity:0},duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}))}else{animit.runAll(maskClear,animit(enterPage.element[0]).queue({css:{transform:"translate3D(-25%, 0px, 0px)",opacity:.9},duration:0}).queue({css:{transform:"translate3D(0px, 0px, 0px)",opacity:1},duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).resetStyle(),animit(leavePage.element[0]).queue({css:{transform:"translate3D(0px, 0px, 0px)"},duration:0}).queue({css:{transform:"translate3D(100%, 0px, 0px)"},duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).queue(function(finish){done();finish()}))}}});return IOSSlideTransitionAnimator}])})();(function(){"use strict;";var module=angular.module("onsen");module.factory("LiftTransitionAnimator",["NavigatorTransitionAnimator",function(NavigatorTransitionAnimator){var LiftTransitionAnimator=NavigatorTransitionAnimator.extend({backgroundMask:angular.element('<div style="position: absolute; width: 100%;'+'height: 100%; background-color: black;"></div>'),push:function(enterPage,leavePage,callback){var mask=this.backgroundMask.remove();leavePage.element[0].parentNode.insertBefore(mask[0],leavePage.element[0]);var maskClear=animit(mask[0]).wait(.6).queue(function(done){mask.remove();done()});animit.runAll(maskClear,animit(enterPage.element[0]).queue({css:{transform:"translate3D(0, 100%, 0)"},duration:0}).queue({css:{transform:"translate3D(0, 0, 0)"},duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).wait(.2).resetStyle().queue(function(done){callback();done()}),animit(leavePage.element[0]).queue({css:{transform:"translate3D(0, 0, 0)",opacity:1},duration:0}).queue({css:{transform:"translate3D(0, -10%, 0)",opacity:.9},duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}))},pop:function(enterPage,leavePage,callback){var mask=this.backgroundMask.remove();enterPage.element[0].parentNode.insertBefore(mask[0],enterPage.element[0]);animit.runAll(animit(mask[0]).wait(.4).queue(function(done){mask.remove();done()}),animit(enterPage.element[0]).queue({css:{transform:"translate3D(0, -10%, 0)",opacity:.9},duration:0}).queue({css:{transform:"translate3D(0, 0, 0)",opacity:1},duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).resetStyle().wait(.4).queue(function(done){callback();done()}),animit(leavePage.element[0]).queue({css:{transform:"translate3D(0, 0, 0)"},duration:0}).queue({css:{transform:"translate3D(0, 100%, 0)"},duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}))}});return LiftTransitionAnimator}])})();(function(){"use strict;";var module=angular.module("onsen");module.factory("ModalView",["$onsen",function($onsen){var ModalView=Class.extend({_element:undefined,_scope:undefined,init:function(scope,element){this._scope=scope;this._element=element;this._scope.$on("$destroy",this._destroy.bind(this));this._deviceBackButtonHandler=$onsen.DeviceBackButtonHandler.create(this._element,this._onDeviceBackButton.bind(this));this.hide()},getDeviceBackButtonHandler:function(){return this._deviceBackButtonHandler},show:function(){this._element.css("display","table")},_isVisible:function(){return this._element[0].clientWidth>0},_onDeviceBackButton:function(){return},hide:function(){this._element.css("display","none")},toggle:function(){if(this._isVisible()){return this.hide.apply(this,arguments)}else{return this.show.apply(this,arguments)}},_destroy:function(){this.emit("destroy",{page:this});this._deviceBackButtonHandler.destroy();this._element=this._scope=null}});MicroEvent.mixin(ModalView);return ModalView}])})();(function(){"use strict;";var module=angular.module("onsen");var NavigatorPageObject=Class.extend({init:function(params){this.page=params.page;this.name=params.page;this.element=params.element;this.pageScope=params.pageScope;this.options=params.options;this.navigator=params.navigator},getPageView:function(){if(!this._pageView){this._pageView=this.element.inheritedData("ons-page");if(!this._pageView){throw new Error("Fail to fetch PageView from ons-page element.")}}return this._pageView},destroy:function(){this.pageScope.$destroy();this.element.remove();this._pageView=null;this.element=null;this.pageScope=null;this.options=null;var index=this.navigator.pages.indexOf(this);if(index!==-1){this.navigator.pages.splice(index,1)}this.navigator=null}});module.factory("NavigatorView",["$http","$parse","$templateCache","$compile","$onsen","$timeout","SimpleSlideTransitionAnimator","NavigatorTransitionAnimator","LiftTransitionAnimator","NullTransitionAnimator","IOSSlideTransitionAnimator","FadeTransitionAnimator",function($http,$parse,$templateCache,$compile,$onsen,$timeout,SimpleSlideTransitionAnimator,NavigatorTransitionAnimator,LiftTransitionAnimator,NullTransitionAnimator,IOSSlideTransitionAnimator,FadeTransitionAnimator){var NavigatorView=Class.extend({_element:undefined,pages:undefined,_scope:undefined,_doorLock:undefined,_profiling:false,init:function(options){options=options||options;this._element=options.element||angular.element(window.document.body);this._scope=options.scope||this._element.scope();this._doorLock=new DoorLock;this.pages=[];this._deviceBackButtonHandler=$onsen.DeviceBackButtonHandler.create(this._element,this._onDeviceBackButton.bind(this));this._scope.$on("$destroy",this._destroy.bind(this))},_destroy:function(){this.emit("destroy",{navigator:this});this.pages.forEach(function(page){page.destroy()});this._deviceBackButtonHandler.destroy();this._deviceBackButtonHandler=null},_onDeviceBackButton:function(event){if(this.pages.length>1){this.popPage()}else{event.callParentHandler()}},_normalizePageElement:function(element){for(var i=0;i<element.length;i++){if(element[i].nodeType===1){return angular.element(element[i])}}throw new Error("invalid state")},_createPageElementAndLinkFunction:function(templateHTML,pageScope,done){var div=document.createElement("div");div.innerHTML=templateHTML.trim();var pageElement=angular.element(div);var hasPage=div.childElementCount===1&&div.childNodes[0].nodeName.toLowerCase()==="ons-page";if(hasPage){pageElement=angular.element(div.childNodes[0])}else{throw new Error('You can not supply no "ons-page" element to "ons-navigator".')}var link=$compile(pageElement);return{element:pageElement,link:function(){link(pageScope);safeApply(pageScope)}};function safeApply(scope){var phase=scope.$root.$$phase;if(phase!=="$apply"&&phase!=="$digest"){scope.$apply()}}},insertPage:function(index,page,options){options=options||{};if(options&&typeof options!="object"){throw new Error("options must be an object. You supplied "+options)}if(this.pages.length===0){return this.pushPage.apply(this,arguments)}this._doorLock.waitUnlock(function(){var unlock=this._doorLock.lock();$onsen.getPageHTMLAsync(page).then(function(templateHTML){var pageScope=this._createPageScope();var object=this._createPageElementAndLinkFunction(templateHTML,pageScope);var element=object.element;var link=object.link;element=this._normalizePageElement(element);var pageObject=this._createPageObject(page,element,pageScope,options);if(this.pages.length>0){index=normalizeIndex(index);this._element[0].insertBefore(element[0],this.pages[index]?this.pages[index].element[0]:null);this.pages.splice(index,0,pageObject);link();setTimeout(function(){if(this.getCurrentPage()!==pageObject){element.css("display","none")}unlock();element=null}.bind(this),1e3/60)}else{this._element.append(element);this.pages.push(pageObject);link();unlock();element=null}}.bind(this),function(){unlock();throw new Error("Page is not found: "+page)})}.bind(this));var normalizeIndex=function(index){if(index<0){index=this.pages.length+index}return index}.bind(this)},pushPage:function(page,options){if(this._profiling){console.time("pushPage")}options=options||{};if(options&&typeof options!="object"){throw new Error("options must be an object. You supplied "+options)}if(this._emitPrePushEvent()){return}this._doorLock.waitUnlock(function(){this._pushPage(page,options)}.bind(this))},_pushPage:function(page,options){var unlock=this._doorLock.lock();var done=function(){unlock();if(this._profiling){console.timeEnd("pushPage")}};$onsen.getPageHTMLAsync(page).then(function(templateHTML){var pageScope=this._createPageScope();var object=this._createPageElementAndLinkFunction(templateHTML,pageScope);setImmediate(function(){this._pushPageDOM(page,object.element,object.link,pageScope,options,done);object=null}.bind(this))}.bind(this),function(){done();throw new Error("Page is not found: "+page)}.bind(this))},getDeviceBackButtonHandler:function(){return this._deviceBackButtonHandler},_getAnimatorOption:function(options){var animator=null;if(options.animation instanceof NavigatorTransitionAnimator){return options.animation}if(typeof options.animation==="string"){animator=NavigatorView._transitionAnimatorDict[options.animation]}if(!animator&&this._element.attr("animation")){animator=NavigatorView._transitionAnimatorDict[this._element.attr("animation")]}if(!animator){animator=NavigatorView._transitionAnimatorDict["default"]}if(!(animator instanceof NavigatorTransitionAnimator)){throw new Error('"animator" is not an instance of NavigatorTransitionAnimator.')}return animator},_createPageScope:function(){return this._scope.$new()},_createPageObject:function(page,element,pageScope,options){options.animator=this._getAnimatorOption(options);return new NavigatorPageObject({page:page,element:element,pageScope:pageScope,options:options,navigator:this})},_pushPageDOM:function(page,element,link,pageScope,options,unlock){if(this._profiling){console.time("pushPageDOM")}unlock=unlock||function(){};options=options||{};element=this._normalizePageElement(element);var pageObject=this._createPageObject(page,element,pageScope,options);var event={enterPage:pageObject,leagePage:this.pages[this.pages.length-1],navigator:this};this.pages.push(pageObject);var done=function(){if(this.pages[this.pages.length-2]){this.pages[this.pages.length-2].element.css("display","none")}if(this._profiling){console.timeEnd("pushPageDOM")}unlock();this.emit("postpush",event);if(typeof options.onTransitionEnd==="function"){options.onTransitionEnd()}element=null}.bind(this);if(this.pages.length>1){var leavePage=this.pages.slice(-2)[0];var enterPage=this.pages.slice(-1)[0];this._element.append(element);link();options.animator.push(enterPage,leavePage,done);element=null}else{this._element.append(element);link();done();element=null}},_emitPrePushEvent:function(){var isCanceled=false;var prePushEvent={navigator:this,currentPage:this.getCurrentPage(),cancel:function(){isCanceled=true}};this.emit("prepush",prePushEvent);return isCanceled},_emitPrePopEvent:function(){var isCanceled=false;var prePopEvent={navigator:this,currentPage:this.getCurrentPage(),cancel:function(){isCanceled=true}};this.emit("prepop",prePopEvent);return isCanceled},popPage:function(options){options=options||{};if(this.pages.length<=1){throw new Error("NavigatorView's page stack is empty.")}if(this._emitPrePopEvent()){return}this._doorLock.waitUnlock(function(){this._popPage(options)}.bind(this))},_popPage:function(options){var unlock=this._doorLock.lock();var leavePage=this.pages.pop();if(this.pages[this.pages.length-1]){this.pages[this.pages.length-1].element.css("display","block")}var enterPage=this.pages[this.pages.length-1];var event={leavePage:leavePage,enterPage:this.pages[this.pages.length-1],navigator:this};var callback=function(){leavePage.destroy();unlock();this.emit("postpop",event);event.leavePage=null;if(typeof options.onTransitionEnd==="function"){options.onTransitionEnd()}}.bind(this);leavePage.options.animator.pop(enterPage,leavePage,callback)},resetToPage:function(page,options){options=options||{};if(!options.animator&&!options.animation){options.animation="none"}var onTransitionEnd=options.onTransitionEnd||function(){};var self=this;options.onTransitionEnd=function(){while(self.pages.length>1){self.pages.shift().destroy()}onTransitionEnd()};this.pushPage(page,options)},getCurrentPage:function(){return this.pages[this.pages.length-1]},getPages:function(){return this.pages},canPopPage:function(){return this.pages.length>1}});NavigatorView._transitionAnimatorDict={"default":$onsen.isAndroid()?new SimpleSlideTransitionAnimator:new IOSSlideTransitionAnimator,slide:$onsen.isAndroid()?new SimpleSlideTransitionAnimator:new IOSSlideTransitionAnimator,lift:new LiftTransitionAnimator,fade:new FadeTransitionAnimator,none:new NullTransitionAnimator};NavigatorView.registerTransitionAnimator=function(name,animator){if(!(animator instanceof NavigatorTransitionAnimator)){throw new Error('"animator" param must be an instance of NavigatorTransitionAnimator')}this._transitionAnimatorDict[name]=animator};MicroEvent.mixin(NavigatorView);return NavigatorView}])})();(function(){"use strict;";var module=angular.module("onsen");module.factory("NavigatorTransitionAnimator",function(){var NavigatorTransitionAnimator=Class.extend({push:function(enterPage,leavePage,callback){callback()},pop:function(enterPage,leavePage,callback){callback()}});return NavigatorTransitionAnimator})})();(function(){"use strict;";var module=angular.module("onsen");module.factory("NullTransitionAnimator",["NavigatorTransitionAnimator",function(NavigatorTransitionAnimator){var NullTransitionAnimator=NavigatorTransitionAnimator.extend({});return NullTransitionAnimator}])})();(function(){"use strict";var module=angular.module("onsen");module.factory("OverlaySlidingMenuAnimator",["SlidingMenuAnimator",function(SlidingMenuAnimator){var OverlaySlidingMenuAnimator=SlidingMenuAnimator.extend({_blackMask:undefined,_isRight:false,_element:false,_menuPage:false,_mainPage:false,_width:false,setup:function(element,mainPage,menuPage,options){options=options||{};this._width=options.width||"90%";this._isRight=!!options.isRight;this._element=element;this._mainPage=mainPage;this._menuPage=menuPage;menuPage.css("box-shadow","0px 0 10px 0px rgba(0, 0, 0, 0.2)");menuPage.css({width:options.width,display:"none",zIndex:2});mainPage.css({zIndex:1});if(this._isRight){menuPage.css({right:"-"+options.width,left:"auto"})}else{menuPage.css({right:"auto",left:"-"+options.width})}this._blackMask=angular.element("<div></div>").css({backgroundColor:"black",top:"0px",left:"0px",right:"0px",bottom:"0px",position:"absolute",display:"none",zIndex:0});element.prepend(this._blackMask)},onResized:function(options){this._menuPage.css("width",options.width);if(this._isRight){this._menuPage.css({right:"-"+options.width,left:"auto"})}else{this._menuPage.css({right:"auto",left:"-"+options.width})}if(options.isOpened){var max=this._menuPage[0].clientWidth;var menuStyle=this._generateMenuPageStyle(max);animit(this._menuPage[0]).queue(menuStyle).play()}},destroy:function(){if(this._blackMask){this._blackMask.remove();this._blackMask=null}this._mainPage.removeAttr("style");this._menuPage.removeAttr("style");this._element=this._mainPage=this._menuPage=null},openMenu:function(callback){this._menuPage.css("display","block");this._blackMask.css("display","block");var max=this._menuPage[0].clientWidth;var menuStyle=this._generateMenuPageStyle(max);var mainPageStyle=this._generateMainPageStyle(max);setTimeout(function(){animit(this._mainPage[0]).queue(mainPageStyle,{duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).queue(function(done){callback();done()}).play();animit(this._menuPage[0]).queue(menuStyle,{duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).play()
}.bind(this),1e3/60)},closeMenu:function(callback){this._blackMask.css({display:"block"});var menuPageStyle=this._generateMenuPageStyle(0);var mainPageStyle=this._generateMainPageStyle(0);setTimeout(function(){animit(this._mainPage[0]).queue(mainPageStyle,{duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).queue(function(done){this._menuPage.css("display","none");callback();done()}.bind(this)).play();animit(this._menuPage[0]).queue(menuPageStyle,{duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).play()}.bind(this),1e3/60)},translateMenu:function(options){this._menuPage.css("display","block");this._blackMask.css({display:"block"});var menuPageStyle=this._generateMenuPageStyle(Math.min(options.maxDistance,options.distance));var mainPageStyle=this._generateMainPageStyle(Math.min(options.maxDistance,options.distance));delete mainPageStyle.opacity;animit(this._menuPage[0]).queue(menuPageStyle).play();if(Object.keys(mainPageStyle).length>0){animit(this._mainPage[0]).queue(mainPageStyle).play()}},_generateMenuPageStyle:function(distance){var max=this._menuPage[0].clientWidth;var x=this._isRight?-distance:distance;var transform="translate3d("+x+"px, 0, 0)";return{transform:transform,"box-shadow":distance===0?"none":"0px 0 10px 0px rgba(0, 0, 0, 0.2)"}},_generateMainPageStyle:function(distance){var max=this._menuPage[0].clientWidth;var opacity=1-.1*distance/max;return{opacity:opacity}},copy:function(){return new OverlaySlidingMenuAnimator}});return OverlaySlidingMenuAnimator}])})();(function(){"use strict;";var module=angular.module("onsen");module.factory("PageView",["$onsen","$parse",function($onsen,$parse){var PageView=Class.extend({_registeredToolbarElement:false,_registeredBottomToolbarElement:false,_nullElement:window.document.createElement("div"),_toolbarElement:null,_bottomToolbarElement:null,init:function(scope,element,attrs){this._scope=scope;this._element=element;this._attrs=attrs;this._registeredToolbarElement=false;this._registeredBottomToolbarElement=false;this._nullElement=window.document.createElement("div");this._toolbarElement=angular.element(this._nullElement);this._bottomToolbarElement=angular.element(this._nullElement);this._clearListener=scope.$on("$destroy",this._destroy.bind(this));this._userDeviceBackButtonListener=angular.noop;if(this._attrs.ngDeviceBackbutton||this._attrs.onDeviceBackbutton){this._deviceBackButtonHandler=$onsen.DeviceBackButtonHandler.create(this._element,this._onDeviceBackButton.bind(this))}},_onDeviceBackButton:function($event){this._userDeviceBackButtonListener($event);if(this._attrs.ngDeviceBackbutton){$parse(this._attrs.ngDeviceBackbutton)(this._scope,{$event:$event})}if(this._attrs.onDeviceBackbutton){with(window)with({event:$event}){eval(this._attrs.onDeviceBackbutton)}}},setDeviceBackButtonHandler:function(callback){if(!this._deviceBackButtonHandler){this._deviceBackButtonHandler=$onsen.DeviceBackButtonHandler.create(this._element,this._onDeviceBackButton.bind(this))}this._userDeviceBackButtonListener=callback},getDeviceBackButtonHandler:function(){return this._deviceBackButtonHandler||null},registerToolbar:function(element){if(this._registeredToolbarElement){throw new Error("This page's toolbar is already registered.")}element.remove();var statusFill=this._element[0].querySelector(".page__status-bar-fill");if(statusFill){angular.element(statusFill).after(element)}else{this._element.prepend(element)}this._toolbarElement=element;this._registeredToolbarElement=true},registerBottomToolbar:function(element){if(this._registeredBottomToolbarElement){throw new Error("This page's bottom-toolbar is already registered.")}element.remove();this._bottomToolbarElement=element;this._registeredBottomToolbarElement=true;var fill=angular.element(document.createElement("div"));fill.addClass("page__bottom-bar-fill");fill.css({width:"0px",height:"0px"});this._element.prepend(fill);this._element.append(element)},registerExtraElement:function(element){if(!this._extraElement){this._extraElement=angular.element("<div></div>");this._extraElement.addClass("page__extra");this._extraElement.css({"z-index":"10001"});this._element.append(this._extraElement)}this._extraElement.append(element.remove())},hasToolbarElement:function(){return!!this._registeredToolbarElement},hasBottomToolbarElement:function(){return!!this._registeredBottomToolbarElement},getContentElement:function(){for(var i=0;i<this._element.length;i++){if(this._element[i].querySelector){var content=this._element[i].querySelector(".page__content");if(content){return content}}}throw Error('fail to get ".page__content" element.')},getToolbarElement:function(){return this._toolbarElement[0]||this._nullElement},getBottomToolbarElement:function(){return this._bottomToolbarElement[0]||this._nullElement},getToolbarLeftItemsElement:function(){return this._toolbarElement[0].querySelector(".left")||this._nullElement},getToolbarCenterItemsElement:function(){return this._toolbarElement[0].querySelector(".center")||this._nullElement},getToolbarRightItemsElement:function(){return this._toolbarElement[0].querySelector(".right")||this._nullElement},getToolbarBackButtonLabelElement:function(){return this._toolbarElement[0].querySelector("ons-back-button .back-button__label")||this._nullElement},_destroy:function(){this.emit("destroy",{page:this});if(this._deviceBackButtonHandler){this._deviceBackButtonHandler.destroy();this._deviceBackButtonHandler=null}this._element=null;this._toolbarElement=null;this._nullElement=null;this._bottomToolbarElement=null;this._extraElement=null;this._scope=null;this._clearListener()}});MicroEvent.mixin(PageView);return PageView}])})();(function(){"use strict";var module=angular.module("onsen");module.factory("PushSlidingMenuAnimator",["SlidingMenuAnimator",function(SlidingMenuAnimator){var PushSlidingMenuAnimator=SlidingMenuAnimator.extend({_isRight:false,_element:undefined,_menuPage:undefined,_mainPage:undefined,_width:undefined,setup:function(element,mainPage,menuPage,options){options=options||{};this._element=element;this._mainPage=mainPage;this._menuPage=menuPage;this._isRight=!!options.isRight;this._width=options.width||"90%";menuPage.css({width:options.width,display:"none"});if(this._isRight){menuPage.css({right:"-"+options.width,left:"auto"})}else{menuPage.css({right:"auto",left:"-"+options.width})}},onResized:function(options){this._menuPage.css("width",options.width);if(this._isRight){this._menuPage.css({right:"-"+options.width,left:"auto"})}else{this._menuPage.css({right:"auto",left:"-"+options.width})}if(options.isOpened){var max=this._menuPage[0].clientWidth;var mainPageTransform=this._generateAbovePageTransform(max);var menuPageStyle=this._generateBehindPageStyle(max);animit(this._mainPage[0]).queue({transform:mainPageTransform}).play();animit(this._menuPage[0]).queue(menuPageStyle).play()}},destroy:function(){this._mainPage.removeAttr("style");this._menuPage.removeAttr("style");this._element=this._mainPage=this._menuPage=null},openMenu:function(callback){this._menuPage.css("display","block");var max=this._menuPage[0].clientWidth;var aboveTransform=this._generateAbovePageTransform(max);var behindStyle=this._generateBehindPageStyle(max);setTimeout(function(){animit(this._mainPage[0]).queue({transform:aboveTransform},{duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).queue(function(done){callback();done()}).play();animit(this._menuPage[0]).queue(behindStyle,{duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).play()}.bind(this),1e3/60)},closeMenu:function(callback){var aboveTransform=this._generateAbovePageTransform(0);var behindStyle=this._generateBehindPageStyle(0);setTimeout(function(){animit(this._mainPage[0]).queue({transform:aboveTransform},{duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).queue({transform:"translate3d(0, 0, 0)"}).queue(function(done){this._menuPage.css("display","none");callback();done()}.bind(this)).play();animit(this._menuPage[0]).queue(behindStyle,{duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).queue(function(done){done()}).play()}.bind(this),1e3/60)},translateMenu:function(options){this._menuPage.css("display","block");var aboveTransform=this._generateAbovePageTransform(Math.min(options.maxDistance,options.distance));var behindStyle=this._generateBehindPageStyle(Math.min(options.maxDistance,options.distance));animit(this._mainPage[0]).queue({transform:aboveTransform}).play();animit(this._menuPage[0]).queue(behindStyle).play()},_generateAbovePageTransform:function(distance){var x=this._isRight?-distance:distance;var aboveTransform="translate3d("+x+"px, 0, 0)";return aboveTransform},_generateBehindPageStyle:function(distance){var max=this._menuPage[0].clientWidth;var behindX=this._isRight?-distance:distance;var behindTransform="translate3d("+behindX+"px, 0, 0)";return{transform:behindTransform}},copy:function(){return new PushSlidingMenuAnimator}});return PushSlidingMenuAnimator}])})();(function(){"use strict";var module=angular.module("onsen");module.factory("RevealSlidingMenuAnimator",["SlidingMenuAnimator",function(SlidingMenuAnimator){var RevealSlidingMenuAnimator=SlidingMenuAnimator.extend({_blackMask:undefined,_isRight:false,_menuPage:undefined,_element:undefined,_mainPage:undefined,setup:function(element,mainPage,menuPage,options){this._element=element;this._menuPage=menuPage;this._mainPage=mainPage;this._isRight=!!options.isRight;this._width=options.width||"90%";mainPage.css({boxShadow:"0px 0 10px 0px rgba(0, 0, 0, 0.2)"});menuPage.css({width:options.width,opacity:.9,display:"none"});if(this._isRight){menuPage.css({right:"0px",left:"auto"})}else{menuPage.css({right:"auto",left:"0px"})}this._blackMask=angular.element("<div></div>").css({backgroundColor:"black",top:"0px",left:"0px",right:"0px",bottom:"0px",position:"absolute",display:"none"});element.prepend(this._blackMask);animit(mainPage[0]).queue({transform:"translate3d(0, 0, 0)"}).play()},onResized:function(options){this._width=options.width;this._menuPage.css("width",this._width);if(options.isOpened){var max=this._menuPage[0].clientWidth;var aboveTransform=this._generateAbovePageTransform(max);var behindStyle=this._generateBehindPageStyle(max);animit(this._mainPage[0]).queue({transform:aboveTransform}).play();animit(this._menuPage[0]).queue(behindStyle).play()}},destroy:function(){if(this._blackMask){this._blackMask.remove();this._blackMask=null}if(this._mainPage){this._mainPage.attr("style","")}if(this._menuPage){this._menuPage.attr("style","")}this._mainPage=this._menuPage=this._element=undefined},openMenu:function(callback){this._menuPage.css("display","block");this._blackMask.css("display","block");var max=this._menuPage[0].clientWidth;var aboveTransform=this._generateAbovePageTransform(max);var behindStyle=this._generateBehindPageStyle(max);setTimeout(function(){animit(this._mainPage[0]).queue({transform:aboveTransform},{duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).queue(function(done){callback();done()}).play();animit(this._menuPage[0]).queue(behindStyle,{duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).play()}.bind(this),1e3/60)},closeMenu:function(callback){this._blackMask.css("display","block");var aboveTransform=this._generateAbovePageTransform(0);var behindStyle=this._generateBehindPageStyle(0);setTimeout(function(){animit(this._mainPage[0]).queue({transform:aboveTransform},{duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).queue({transform:"translate3d(0, 0, 0)"}).queue(function(done){this._menuPage.css("display","none");callback();done()}.bind(this)).play();animit(this._menuPage[0]).queue(behindStyle,{duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).queue(function(done){done()}).play()}.bind(this),1e3/60)},translateMenu:function(options){this._menuPage.css("display","block");this._blackMask.css("display","block");var aboveTransform=this._generateAbovePageTransform(Math.min(options.maxDistance,options.distance));var behindStyle=this._generateBehindPageStyle(Math.min(options.maxDistance,options.distance));delete behindStyle.opacity;animit(this._mainPage[0]).queue({transform:aboveTransform}).play();animit(this._menuPage[0]).queue(behindStyle).play()},_generateAbovePageTransform:function(distance){var x=this._isRight?-distance:distance;var aboveTransform="translate3d("+x+"px, 0, 0)";return aboveTransform},_generateBehindPageStyle:function(distance){var max=this._menuPage[0].getBoundingClientRect().width;var behindDistance=(distance-max)/max*10;behindDistance=isNaN(behindDistance)?0:Math.max(Math.min(behindDistance,0),-10);var behindX=this._isRight?-behindDistance:behindDistance;var behindTransform="translate3d("+behindX+"%, 0, 0)";var opacity=1+behindDistance/100;return{transform:behindTransform,opacity:opacity}},copy:function(){return new RevealSlidingMenuAnimator}});return RevealSlidingMenuAnimator}])})();(function(){"use strict;";var module=angular.module("onsen");module.factory("SimpleSlideTransitionAnimator",["NavigatorTransitionAnimator",function(NavigatorTransitionAnimator){var SimpleSlideTransitionAnimator=NavigatorTransitionAnimator.extend({backgroundMask:angular.element('<div style="position: absolute; width: 100%;'+'height: 100%; background-color: black; opacity: 0;"></div>'),timing:"cubic-bezier(.1, .7, .4, 1)",duration:.3,blackMaskOpacity:.4,init:function(options){options=options||{};this.timing=options.timing||this.timing;this.duration=options.duration!==undefined?options.duration:this.duration},push:function(enterPage,leavePage,callback){var mask=this.backgroundMask.remove();leavePage.element[0].parentNode.insertBefore(mask[0],leavePage.element[0].nextSibling);animit.runAll(animit(mask[0]).queue({opacity:0,transform:"translate3d(0, 0, 0)"}).queue({opacity:this.blackMaskOpacity},{duration:this.duration,timing:this.timing}).resetStyle().queue(function(done){mask.remove();done()}),animit(enterPage.element[0]).queue({css:{transform:"translate3D(100%, 0, 0)"},duration:0}).queue({css:{transform:"translate3D(0, 0, 0)"},duration:this.duration,timing:this.timing}).resetStyle(),animit(leavePage.element[0]).queue({css:{transform:"translate3D(0, 0, 0)"},duration:0}).queue({css:{transform:"translate3D(-45%, 0px, 0px)"},duration:this.duration,timing:this.timing}).resetStyle().wait(.2).queue(function(done){callback();done()}))},pop:function(enterPage,leavePage,done){var mask=this.backgroundMask.remove();enterPage.element[0].parentNode.insertBefore(mask[0],enterPage.element[0].nextSibling);animit.runAll(animit(mask[0]).queue({opacity:this.blackMaskOpacity,transform:"translate3d(0, 0, 0)"}).queue({opacity:0},{duration:this.duration,timing:this.timing}).resetStyle().queue(function(done){mask.remove();done()}),animit(enterPage.element[0]).queue({css:{transform:"translate3D(-45%, 0px, 0px)",opacity:.9},duration:0}).queue({css:{transform:"translate3D(0px, 0px, 0px)",opacity:1},duration:this.duration,timing:this.timing}).resetStyle(),animit(leavePage.element[0]).queue({css:{transform:"translate3D(0px, 0px, 0px)"},duration:0}).queue({css:{transform:"translate3D(100%, 0px, 0px)"},duration:this.duration,timing:this.timing}).wait(.2).queue(function(finish){done();finish()}))}});return SimpleSlideTransitionAnimator}])})();(function(){"use strict";var module=angular.module("onsen");var SlidingMenuViewModel=Class.extend({_distance:0,_maxDistance:undefined,init:function(options){if(!angular.isNumber(options.maxDistance)){throw new Error("options.maxDistance must be number")}this.setMaxDistance(options.maxDistance)},setMaxDistance:function(maxDistance){if(maxDistance<=0){throw new Error("maxDistance must be greater then zero.")}if(this.isOpened()){this._distance=maxDistance}this._maxDistance=maxDistance},shouldOpen:function(){return!this.isOpened()&&this._distance>=this._maxDistance/2},shouldClose:function(){return!this.isClosed()&&this._distance<this._maxDistance/2},openOrClose:function(callback){if(this.shouldOpen()){this.open(callback)}else if(this.shouldClose()){this.close(callback)}},close:function(callback){callback=callback||function(){};if(!this.isClosed()){this._distance=0;this.emit("close",{callback:callback})}else{callback()}},open:function(callback){callback=callback||function(){};if(!this.isOpened()){this._distance=this._maxDistance;this.emit("open",{callback:callback})}else{callback()}},isClosed:function(){return this._distance===0},isOpened:function(){return this._distance===this._maxDistance},getX:function(){return this._distance},getMaxDistance:function(){return this._maxDistance},translate:function(x){this._distance=Math.max(1,Math.min(this._maxDistance-1,x));var options={distance:this._distance,maxDistance:this._maxDistance};this.emit("translate",options)},toggle:function(){if(this.isClosed()){this.open()}else{this.close()}}});MicroEvent.mixin(SlidingMenuViewModel);var MAIN_PAGE_RATIO=.9;module.factory("SlidingMenuView",["$onsen","$compile","SlidingMenuAnimator","RevealSlidingMenuAnimator","PushSlidingMenuAnimator","OverlaySlidingMenuAnimator",function($onsen,$compile,SlidingMenuAnimator,RevealSlidingMenuAnimator,PushSlidingMenuAnimator,OverlaySlidingMenuAnimator){var SlidingMenuView=Class.extend({_scope:undefined,_attrs:undefined,_element:undefined,_behindPage:undefined,_abovePage:undefined,_doorLock:undefined,_isRightMenu:false,init:function(scope,element,attrs){this._scope=scope;this._attrs=attrs;this._element=element;this._behindPage=angular.element(element[0].querySelector(".onsen-sliding-menu__behind"));this._abovePage=angular.element(element[0].querySelector(".onsen-sliding-menu__above"));this._doorLock=new DoorLock;this._isRightMenu=attrs.side==="right";var maxDistance=this._normalizeMaxSlideDistanceAttr();this._logic=new SlidingMenuViewModel({maxDistance:Math.max(maxDistance,1)});this._logic.on("translate",this._translate.bind(this));this._logic.on("open",function(options){this._open(options.callback)}.bind(this));this._logic.on("close",function(options){this._close(options.callback)}.bind(this));attrs.$observe("maxSlideDistance",this._onMaxSlideDistanceChanged.bind(this));attrs.$observe("swipable",this._onSwipableChanged.bind(this));window.addEventListener("resize",this._onWindowResize.bind(this));this._boundHandleEvent=this._handleEvent.bind(this);this._bindEvents();if(attrs.mainPage){this.setMainPage(attrs.mainPage)}else if(attrs.abovePage){this.setMainPage(attrs.abovePage)}if(attrs.menuPage){this.setMenuPage(attrs.menuPage)}else if(attrs.behindPage){this.setMenuPage(attrs.behindPage)}this._deviceBackButtonHandler=$onsen.DeviceBackButtonHandler.create(this._element,this._onDeviceBackButton.bind(this));var unlock=this._doorLock.lock();window.setTimeout(function(){var maxDistance=this._normalizeMaxSlideDistanceAttr();this._logic.setMaxDistance(maxDistance);unlock();this._behindPage.css({opacity:1});this._animator=this._getAnimatorOption();this._animator.setup(this._element,this._abovePage,this._behindPage,{isRight:this._isRightMenu,width:this._attrs.maxSlideDistance||"90%"})}.bind(this),400);scope.$on("$destroy",this._destroy.bind(this))},getDeviceBackButtonHandler:function(){return this._deviceBackButtonHandler},_onDeviceBackButton:function(event){if(this.isMenuOpened()){this.closeMenu()}else{event.callParentHandler()}},_refreshBehindPageWidth:function(){var width="maxSlideDistance"in this._attrs?this._attrs.maxSlideDistance:"90%";if(this._animator){this._animator.onResized({isOpened:this._logic.isOpened(),width:width})}},_destroy:function(){this.emit("destroy",{slidingMenu:this});this._deviceBackButtonHandler.destroy();this._element=this._scope=this._attrs=null},_getAnimatorOption:function(){var animator=SlidingMenuView._animatorDict[this._attrs.type];if(!(animator instanceof SlidingMenuAnimator)){animator=SlidingMenuView._animatorDict["default"]}return animator.copy()},_onSwipableChanged:function(swipable){swipable=swipable===""||swipable===undefined||swipable=="true";this.setSwipable(swipable)},setSwipable:function(enabled){if(enabled){this._activateHammer()}else{this._deactivateHammer()}},_onWindowResize:function(){this._recalculateMAX();this._refreshBehindPageWidth()},_onMaxSlideDistanceChanged:function(){this._recalculateMAX();this._refreshBehindPageWidth()},_normalizeMaxSlideDistanceAttr:function(){var maxDistance=this._attrs.maxSlideDistance;if(!("maxSlideDistance"in this._attrs)){maxDistance=.9*this._abovePage[0].clientWidth}else if(typeof maxDistance=="string"){if(maxDistance.indexOf("px",maxDistance.length-2)!==-1){maxDistance=parseInt(maxDistance.replace("px",""),10)}else if(maxDistance.indexOf("%",maxDistance.length-1)>0){maxDistance=maxDistance.replace("%","");maxDistance=parseFloat(maxDistance)/100*this._abovePage[0].clientWidth}}else{throw new Error("invalid state")}return maxDistance},_recalculateMAX:function(){var maxDistance=this._normalizeMaxSlideDistanceAttr();if(maxDistance){this._logic.setMaxDistance(parseInt(maxDistance,10))}},_activateHammer:function(){this._hammertime.on("touch dragleft dragright swipeleft swiperight release",this._boundHandleEvent)},_deactivateHammer:function(){this._hammertime.off("touch dragleft dragright swipeleft swiperight release",this._boundHandleEvent)},_bindEvents:function(){this._hammertime=new Hammer(this._element[0])},_appendAbovePage:function(pageUrl,templateHTML){var pageScope=this._scope.$parent.$new();var pageContent=angular.element(templateHTML);var link=$compile(pageContent);this._abovePage.append(pageContent);if(this._currentPageElement){this._currentPageElement.remove();this._currentPageScope.$destroy()}link(pageScope);this._currentPageElement=pageContent;this._currentPageScope=pageScope;this._currentPageUrl=pageUrl},_appendBehindPage:function(templateHTML){var pageScope=this._scope.$parent.$new();var pageContent=angular.element(templateHTML);var link=$compile(pageContent);this._behindPage.append(pageContent);if(this._currentBehindPageScope){this._currentBehindPageScope.$destroy();this._currentBehindPageElement.remove()}link(pageScope);this._currentBehindPageElement=pageContent;this._currentBehindPageScope=pageScope},setMenuPage:function(page,options){if(page){options=options||{};options.callback=options.callback||function(){};var self=this;$onsen.getPageHTMLAsync(page).then(function(html){self._appendBehindPage(angular.element(html));if(options.closeMenu){self.close()}options.callback()},function(){throw new Error("Page is not found: "+page)})}else{throw new Error("cannot set undefined page")}},setBehindPage:function(){return this.setMenuPage.apply(this,arguments)},setMainPage:function(pageUrl,options){options=options||{};options.callback=options.callback||function(){};var done=function(){if(options.closeMenu){this.close()}options.callback()}.bind(this);if(this.currentPageUrl===pageUrl){done();return}if(pageUrl){var self=this;$onsen.getPageHTMLAsync(pageUrl).then(function(html){self._appendAbovePage(pageUrl,html);done()},function(){throw new Error("Page is not found: "+page)})}else{throw new Error("cannot set undefined page")}},setAbovePage:function(pageUrl,options){return this.setMainPage.apply(this,arguments)},_handleEvent:function(event){if(this._doorLock.isLocked()){return}if(this._isInsideIgnoredElement(event.target)){event.gesture.stopDetect()}switch(event.type){case"touch":if(this._logic.isClosed()){if(!this._isInsideSwipeTargetArea(event)){event.gesture.stopDetect()}}break;case"dragleft":case"dragright":event.gesture.preventDefault();var deltaX=event.gesture.deltaX;var deltaDistance=this._isRightMenu?-deltaX:deltaX;var startEvent=event.gesture.startEvent;if(!("isOpened"in startEvent)){startEvent.isOpened=this._logic.isOpened()}if(deltaDistance<0&&this._logic.isClosed()){break}if(deltaDistance>0&&this._logic.isOpened()){break}var distance=startEvent.isOpened?deltaDistance+this._logic.getMaxDistance():deltaDistance;this._logic.translate(distance);break;case"swipeleft":event.gesture.preventDefault();if(this._isRightMenu){this.open()}else{this.close()}event.gesture.stopDetect();break;case"swiperight":event.gesture.preventDefault();if(this._isRightMenu){this.close()}else{this.open()}event.gesture.stopDetect();break;case"release":this._lastDistance=null;if(this._logic.shouldOpen()){this.open()}else if(this._logic.shouldClose()){this.close()}break}},_isInsideIgnoredElement:function(element){do{if(element.getAttribute&&element.getAttribute("sliding-menu-ignore")){return true}element=element.parentNode}while(element);return false},_isInsideSwipeTargetArea:function(event){var x=event.gesture.center.pageX;if(!("_swipeTargetWidth"in event.gesture.startEvent)){event.gesture.startEvent._swipeTargetWidth=this._getSwipeTargetWidth()}var targetWidth=event.gesture.startEvent._swipeTargetWidth;return this._isRightMenu?this._abovePage[0].clientWidth-x<targetWidth:x<targetWidth},_getSwipeTargetWidth:function(){var targetWidth=this._attrs.swipeTargetWidth;if(typeof targetWidth=="string"){targetWidth=targetWidth.replace("px","")}var width=parseInt(targetWidth,10);if(width<0||!targetWidth){return this._abovePage[0].clientWidth}else{return width}},closeMenu:function(){return this.close.apply(this,arguments)},close:function(callback){callback=callback||function(){};this.emit("preclose");this._doorLock.waitUnlock(function(){this._logic.close(callback)}.bind(this))},_close:function(callback){callback=callback||function(){};var unlock=this._doorLock.lock();this._animator.closeMenu(function(){unlock();this.emit("postclose");callback()}.bind(this))},openMenu:function(){return this.open.apply(this,arguments)},open:function(callback){callback=callback||function(){};this.emit("preopen");this._doorLock.waitUnlock(function(){this._logic.open(callback)}.bind(this))},_open:function(callback){callback=callback||function(){};var unlock=this._doorLock.lock();this._animator.openMenu(function(){unlock();this.emit("postopen");callback()}.bind(this))},toggle:function(callback){if(this._logic.isClosed()){this.open(callback)}else{this.close(callback)}},toggleMenu:function(){return this.toggle.apply(this,arguments)},isMenuOpened:function(){return this._logic.isOpened()},_translate:function(event){this._animator.translateMenu(event)}});SlidingMenuView._animatorDict={"default":new RevealSlidingMenuAnimator,overlay:new OverlaySlidingMenuAnimator,reveal:new RevealSlidingMenuAnimator,push:new PushSlidingMenuAnimator};SlidingMenuView.registerSlidingMenuAnimator=function(name,animator){if(!(animator instanceof SlidingMenuAnimator)){throw new Error('"animator" param must be an instance of SlidingMenuAnimator')}this._animatorDict[name]=animator};MicroEvent.mixin(SlidingMenuView);return SlidingMenuView}])})();(function(){"use strict";var module=angular.module("onsen");module.factory("SlidingMenuAnimator",function(){return Class.extend({setup:function(element,mainPage,menuPage,options){},onResized:function(options){},openMenu:function(callback){},closeClose:function(callback){},destroy:function(){},translateMenu:function(mainPage,menuPage,options){},copy:function(){throw new Error("Override copy method.")}})})})();(function(){"use strict";var module=angular.module("onsen");module.factory("SplitView",["$compile","RevealSlidingMenuAnimator","$onsen",function($compile,RevealSlidingMenuAnimator,$onsen){var SPLIT_MODE=0;var COLLAPSE_MODE=1;var MAIN_PAGE_RATIO=.9;var ON_PAGE_READY="onPageReady";var SplitView=Class.extend({init:function(scope,element){element.addClass("onsen-sliding-menu");this._element=element;this._scope=scope;this._abovePage=angular.element(element[0].querySelector(".onsen-split-view__main"));this._behindPage=angular.element(element[0].querySelector(".onsen-split-view__secondary"));this._previousX=0;this._max=this._abovePage[0].clientWidth*MAIN_PAGE_RATIO;this._currentX=0;this._startX=0;this._mode=SPLIT_MODE;this._doorLock=new DoorLock;this._hammertime=new Hammer(this._element[0]);this._boundHammerEvent=this._handleEvent.bind(this);scope.$watch("swipable",this._onSwipableChanged.bind(this));if($onsen.isIOS()){window.addEventListener("orientationchange",this._onResize.bind(this))}else{window.addEventListener("resize",this._onResize.bind(this))}this._animator=new RevealSlidingMenuAnimator;this._element.css("display","none");if(scope.mainPage){this.setMainPage(scope.mainPage)}if(scope.secondaryPage){this.setSecondaryPage(scope.secondaryPage)}var unlock=this._doorLock.lock();this._considerChangingCollapse();this._setSize();setTimeout(function(){this._element.css("display","block");unlock()}.bind(this),1e3/60*2);scope.$on("$destroy",this._destroy.bind(this))},_appendSecondPage:function(templateHTML){var pageScope=this._scope.$parent.$new();var pageContent=$compile(templateHTML)(pageScope);this._behindPage.append(pageContent);if(this._currentBehindPageElement){this._currentBehindPageElement.remove();this._currentBehindPageScope.$destroy()}this._currentBehindPageElement=pageContent;this._currentBehindPageScope=pageScope},_appendMainPage:function(templateHTML){var pageScope=this._scope.$parent.$new();var pageContent=$compile(templateHTML)(pageScope);this._abovePage.append(pageContent);if(this._currentPage){this._currentPage.remove();this._currentPageScope.$destroy()}this._currentPage=pageContent;this._currentPageScope=pageScope},setSecondaryPage:function(page){if(page){$onsen.getPageHTMLAsync(page).then(function(html){this._appendSecondPage(angular.element(html.trim()))}.bind(this),function(){throw new Error("Page is not found: "+page)})}else{throw new Error("cannot set undefined page")}},setMainPage:function(page){if(page){$onsen.getPageHTMLAsync(page).then(function(html){this._appendMainPage(angular.element(html.trim()))}.bind(this),function(){throw new Error("Page is not found: "+page)})}else{throw new Error("cannot set undefined page")}},_onResize:function(){var lastMode=this._mode;this._considerChangingCollapse();if(lastMode===COLLAPSE_MODE&&this._mode===COLLAPSE_MODE){this._animator.onResized({isOpened:this._currentX>0,width:"90%"})}this._max=this._abovePage[0].clientWidth*MAIN_PAGE_RATIO},_considerChangingCollapse:function(){if(this._shouldCollapse()){this._activateCollapseMode()}else{this._activateSplitMode()}},_shouldCollapse:function(){var orientation=window.orientation;if(orientation===undefined){orientation=window.innerWidth>window.innerHeight?90:0}switch(this._scope.collapse){case undefined:case"none":return false;case"portrait":return orientation===180||orientation===0;case"landscape":return orientation==90||orientation==-90;default:if(this._scope.collapse===undefined){return false}var widthToken;if(this._scope.collapse.indexOf("width")>=0){var tokens=this._scope.collapse.split(" ");widthToken=tokens[tokens.length-1]}else{widthToken=this._scope.collapse}if(widthToken.indexOf("px")>0){widthToken=widthToken.substr(0,widthToken.length-2)}return isNumber(widthToken)&&window.innerWidth<widthToken}},_setSize:function(){if(this._mode===SPLIT_MODE){if(!this._scope.mainPageWidth){this._scope.mainPageWidth="70"}var behindSize=100-this._scope.mainPageWidth.replace("%","");this._behindPage.css({width:behindSize+"%",opacity:1});this._abovePage.css({width:this._scope.mainPageWidth+"%"});this._abovePage.css("left",behindSize+"%");this._currentX=this._behindPage[0].clientWidth}},_activateCollapseMode:function(){if(this._mode!==COLLAPSE_MODE){this._behindPage.attr("style","");this._abovePage.attr("style","");this._mode=COLLAPSE_MODE;this._onSwipableChanged(this._scope.swipable);this._animator.setup(this._element,this._abovePage,this._behindPage,{isRight:false,width:"90%"});this._currentX=this._startX=0}},_activateSplitMode:function(){this._animator.destroy();this._behindPage.attr("style","");this._abovePage.attr("style","");this._mode=SPLIT_MODE;this._setSize();this._deactivateHammer()},_activateHammer:function(){this._hammertime.on("dragleft dragright swipeleft swiperight release",this._boundHammerEvent)},_deactivateHammer:function(){this._hammertime.off("dragleft dragright swipeleft swiperight release",this._boundHammerEvent)},_onSwipableChanged:function(swipable){swipable=swipable===""||swipable===undefined||swipable=="true";if(swipable){this._activateHammer()}else{this._deactivateHammer()}},_handleEvent:function(event){if(this._doorLock.isLocked()){return
}switch(event.type){case"dragleft":case"dragright":event.preventDefault();event.gesture.preventDefault();var deltaX=event.gesture.deltaX;this._currentX=this._startX+deltaX;if(this._currentX>=0){this._translate(this._currentX)}break;case"swipeleft":event.gesture.preventDefault();this.close();break;case"swiperight":event.gesture.preventDefault();this.open();break;case"release":if(this._currentX>this._max/2){this.open()}else{this.close()}break}},_onTransitionEnd:function(){this._scope.$root.$broadcast(ON_PAGE_READY)},close:function(callback){callback=callback||function(){};if(this._mode===SPLIT_MODE){callback();return}else if(this._mode===COLLAPSE_MODE){this._startX=0;if(this._currentX!==0){var self=this;this._doorLock.waitUnlock(function(){var unlock=self._doorLock.lock();self._currentX=0;self._animator.closeMenu(function(){unlock();self._onTransitionEnd();callback()})})}}},open:function(callback){callback=callback||function(){};if(this._mode===SPLIT_MODE){callback();return}else if(this._mode===COLLAPSE_MODE){this._startX=this._max;if(this._currentX!=this._max){var self=this;this._doorLock.waitUnlock(function(){var unlock=self._doorLock.lock();self._currentX=self._max;self._animator.openMenu(function(){unlock();self._onTransitionEnd();callback()})})}}},toggle:function(callback){if(this._startX===0){this.open(callback)}else{this.close(callback)}},_translate:function(x){if(this._mode===COLLAPSE_MODE){this._currentX=x;var options={distance:x,maxDistance:this._max};this._animator.translateMenu(options)}},_destroy:function(){this.emit("destroy",{splitView:this});this._element=null;this._scope=null}});function isNumber(n){return!isNaN(parseFloat(n))&&isFinite(n)}MicroEvent.mixin(SplitView);return SplitView}])})();(function(){"use strict";var module=angular.module("onsen");module.factory("SwitchView",["$onsen",function($onsen){var SwitchView=Class.extend({init:function(element,scope,attrs){this._element=element;this._checkbox=angular.element(element[0].querySelector("input[type=checkbox]"));this._scope=scope;attrs.$observe("disabled",function(disabled){if(!!element.attr("disabled")){this._checkbox.attr("disabled","disabled")}else{this._checkbox.removeAttr("disabled")}}.bind(this));scope.$watch("model",function(model){this.emit("change",{"switch":this,value:!!model})}.bind(this))},isChecked:function(){return this._checkbox[0].checked},setChecked:function(isChecked){this._scope.model=!!isChecked;this._scope.$evalAsync()},getCheckboxElemenet:function(){return this._checkbox[0]}});MicroEvent.mixin(SwitchView);return SwitchView}])})();(function(){"use strict;";var module=angular.module("onsen");module.factory("TabbarAnimator",function(){var TabbarAnimator=Class.extend({apply:function(enterPage,leavePage,done){throw new Error("This method must be implemented.")}});return TabbarAnimator});module.factory("TabbarNoneAnimator",["TabbarAnimator",function(TabbarAnimator){var TabbarNoneAnimator=TabbarAnimator.extend({apply:function(enterPage,leavePage,done){done()}});return TabbarNoneAnimator}]);module.factory("TabbarFadeAnimator",["TabbarAnimator",function(TabbarAnimator){var TabbarFadeAnimator=TabbarAnimator.extend({apply:function(enterPage,leavePage,done){animit.runAll(animit(enterPage[0]).queue({transform:"translate3D(0, 0, 0)",opacity:0}).queue({transform:"translate3D(0, 0, 0)",opacity:1},{duration:.4,timing:"linear"}).resetStyle().queue(function(callback){done();callback()}),animit(leavePage[0]).queue({transform:"translate3D(0, 0, 0)",opacity:1}).queue({transform:"translate3D(0, 0, 0)",opacity:0},{duration:.4,timing:"linear"}))}});return TabbarFadeAnimator}]);module.factory("TabbarView",["$onsen","$compile","TabbarAnimator","TabbarNoneAnimator","TabbarFadeAnimator",function($onsen,$compile,TabbarAnimator,TabbarNoneAnimator,TabbarFadeAnimator){var TabbarView=Class.extend({_tabbarId:undefined,_tabItems:undefined,init:function(scope,element,attrs){this._scope=scope;this._element=element;this._attrs=attrs;this._tabbarId=Date.now();this._tabItems=[];this._containerElement=angular.element(element[0].querySelector(".ons-tab-bar__content"));this._footerElement=angular.element(element[0].querySelector(".ons-tab-bar__footer"));this._scope.$on("$destroy",this._destroy.bind(this))},setActiveTab:function(index,options){options=options||{};var previousTabItem=this._tabItems[this.getActiveTabIndex()];var selectedTabItem=this._tabItems[index];var needLoad=selectedTabItem.page&&!options.keepPage;if(!selectedTabItem){return false}var canceled=false;this.emit("prechange",{index:index,tabItem:selectedTabItem,cancel:function(){canceled=true}});if(canceled){selectedTabItem.setInactive();if(previousTabItem){previousTabItem.setActive()}return false}selectedTabItem.setActive();if(needLoad){this._loadPage(selectedTabItem.page,{callback:function(){this.emit("postchange",{index:index,tabItem:selectedTabItem})}.bind(this)})}for(var i=0;i<this._tabItems.length;i++){if(this._tabItems[i]!=selectedTabItem){this._tabItems[i].setInactive()}else{this._triggerActiveTabChanged(i,selectedTabItem);if(!needLoad){this.emit("postchange",{index:index,tabItem:selectedTabItem})}}}return true},_triggerActiveTabChanged:function(index,tabItem){this._scope.onActiveTabChanged({$index:index,$tabItem:tabItem})},setTabbarVisibility:function(visible){this._scope.hideTabs=!visible;this._onTabbarVisibilityChanged()},_onTabbarVisibilityChanged:function(){if(this._scope.hideTabs){this._scope.tabbarHeight=0}else{this._scope.tabbarHeight=this._footerElement[0].clientHeight+"px"}},addTabItem:function(tabItem){this._tabItems.push(tabItem)},getActiveTabIndex:function(){var tabItem;for(var i=0;i<this._tabItems.length;i++){tabItem=this._tabItems[i];if(tabItem.isActive()){return i}}return-1},loadPage:function(page,options){return this._loadPage(page,options)},_loadPage:function(page,options){$onsen.getPageHTMLAsync(page).then(function(html){var pageElement=angular.element(html.trim());this._loadPageDOM(page,pageElement,options)}.bind(this),function(){throw new Error("Page is not found: "+page)})},_loadPageDOM:function(page,element,options){options=options||{};var pageScope=this._scope.$parent.$new();var link=$compile(element);this._containerElement.append(element);var pageContent=link(pageScope);pageScope.$evalAsync();if(this._currentPageElement){var oldPageElement=this._currentPageElement;var oldPageScope=this._currentPageScope;this._currentPageElement=pageContent;this._currentPageScope=pageScope;this._getAnimatorOption(options).apply(pageContent,oldPageElement,function(){oldPageElement.remove();oldPageScope.$destroy();if(options.callback instanceof Function){options.callback()}})}else{this._currentPageElement=pageContent;this._currentPageScope=pageScope;if(options.callback instanceof Function){options.callback()}}},_getAnimatorOption:function(options){var animationAttr=this._element.attr("animation")||"default";return TabbarView._animatorDict[options.animation||animationAttr]||TabbarView._animatorDict["default"]},_destroy:function(){this.emit("destroy",{tabbar:this});this._element=this._scope=this._attrs=null}});MicroEvent.mixin(TabbarView);TabbarView._animatorDict={"default":new TabbarNoneAnimator,none:new TabbarNoneAnimator,fade:new TabbarFadeAnimator};TabbarView.registerAnimator=function(name,animator){if(!(animator instanceof TabbarAnimator)){throw new Error('"animator" param must be an instance of TabbarAnimator')}this._transitionAnimatorDict[name]=animator};return TabbarView}])})();(function(){"use strict";var module=angular.module("onsen");module.directive("onsBackButton",["$onsen","$compile","ComponentCleaner",function($onsen,$compile,ComponentCleaner){return{restrict:"E",replace:false,templateUrl:$onsen.DIRECTIVE_TEMPLATE_URL+"/back_button.tpl",transclude:true,scope:true,link:{pre:function(scope,element,attrs,controller,transclude){scope.modifierTemplater=$onsen.generateModifierTemplater(attrs);transclude(scope,function(clonedElement){if(clonedElement[0]){element[0].querySelector(".back-button__label").appendChild(clonedElement[0])}});ComponentCleaner.onDestroy(scope,function(){ComponentCleaner.destroyScope(scope);ComponentCleaner.destroyAttributes(attrs);element=null;scope=null;attrs=null})}}}}])})();(function(){"use strict";var module=angular.module("onsen");module.directive("onsBottomToolbar",["$onsen",function($onsen){return{restrict:"E",replace:false,transclude:false,scope:false,compile:function(element,attrs){var modifierTemplater=$onsen.generateModifierTemplater(attrs);element.addClass("bottom-bar");element.addClass(modifierTemplater("bottom-bar--*"));element.css({"z-index":0});return{pre:function(scope,element,attrs){scope.modifierTemplater=$onsen.generateModifierTemplater(attrs);var pageView=element.inheritedData("ons-page");if(pageView){pageView.registerBottomToolbar(element)}}}}}}])})();(function(){"use strict";var module=angular.module("onsen");module.directive("onsButton",["$onsen",function($onsen){return{restrict:"E",replace:false,transclude:true,scope:{animation:"@"},templateUrl:$onsen.DIRECTIVE_TEMPLATE_URL+"/button.tpl",link:function(scope,element,attrs,_,transclude){var initialAnimation="slide-left";scope.modifierTemplater=$onsen.generateModifierTemplater(attrs);element.addClass("button effeckt-button");element.addClass(scope.modifierTemplater("button--*"));element.addClass(initialAnimation);transclude(scope,function(cloned){angular.element(element[0].querySelector(".ons-button-inner")).append(cloned)});if(attrs.ngController){throw new Error("This element can't accept ng-controller directive.")}scope.item={};scope.item.animation=initialAnimation;attrs.$observe("disabled",function(disabled){if(disabled==="true"){element.attr("disabled",true)}else{element.attr("disabled",false)}});scope.$watch("animation",function(newAnimation){if(newAnimation){if(scope.item.animation){element.removeClass(scope.item.animation)}scope.item.animation=newAnimation;element.addClass(scope.item.animation)}});attrs.$observe("shouldSpin",function(shouldSpin){if(shouldSpin==="true"){element.attr("data-loading",true)}else{element.removeAttr("data-loading")}});$onsen.cleaner.onDestroy(scope,function(){$onsen.clearComponent({scope:scope,attrs:attrs,element:element});scope=element=attrs=null})}}}])})();(function(){"use strict";var module=angular.module("onsen");module.directive("onsCol",["$timeout","$onsen",function($timeout,$onsen){return{restrict:"E",replace:false,transclude:false,scope:false,compile:function(element,attrs,transclude){element.addClass("col ons-col-inner");return function(scope,element,attrs){attrs.$observe("align",function(align){updateAlign(align)});attrs.$observe("width",function(width){updateWidth(width)});attrs.$observe("size",function(size){if(!attrs.width){updateWidth(size)}});updateAlign(attrs.align);if(attrs.size&&!attrs.width){updateWidth(attrs.size)}else{updateWidth(attrs.width)}$onsen.cleaner.onDestroy(scope,function(){$onsen.clearComponent({scope:scope,element:element,attrs:attrs});element=attrs=scope=null});function updateAlign(align){if(align==="top"||align==="center"||align==="bottom"){element.removeClass("col-top col-center col-bottom");element.addClass("col-"+align)}else{element.removeClass("col-top col-center col-bottom")}}function updateWidth(width){if(typeof width==="string"){width=(""+width).trim();width=width.match(/^\d+$/)?width+"%":width;element.css({"-webkit-box-flex":"0","-webkit-flex":"0 0 "+width,"-moz-box-flex":"0","-moz-flex":"0 0 "+width,"-ms-flex":"0 0 "+width,flex:"0 0 "+width,"max-width":width})}else{element.removeAttr("style")}}}}}}])})();(function(){"use strict";var module=angular.module("onsen");module.directive("onsDummyForInit",["$rootScope",function($rootScope){var isReady=false;return{restrict:"E",replace:false,link:{post:function(scope,element){if(!isReady){isReady=true;$rootScope.$broadcast("$ons-ready")}element.remove()}}}}])})();(function(){"use strict";var EVENTS=("drag dragleft dragright dragup dragdown hold release swipe swipeleft swiperight "+"swipeup swipedown tap doubletap touch transform pinch pinchin pinchout rotate").split(/ +/);angular.module("onsen").directive("onsGestureDetector",["$onsen",function($onsen){var scopeDef=EVENTS.reduce(function(dict,name){dict["ng"+titlize(name)]="&";return dict},{});return{restrict:"E",scope:scopeDef,replace:false,transclude:true,compile:function(element,attrs){return function link(scope,element,attrs,controller,transclude){transclude(scope.$parent.$new(),function(cloned){element.append(cloned)});var hammer=new Hammer(element[0]);hammer.on(EVENTS.join(" "),handleEvent);$onsen.cleaner.onDestroy(scope,function(){hammer.off(EVENTS.join(" "),handleEvent);$onsen.clearComponent({scope:scope,element:element,attrs:attrs});hammer.element=scope=element=attrs=null});function handleEvent(event){var attr="ng"+titlize(event.type);if(attr in scopeDef){scope[attr]({$event:event})}}}}};function titlize(str){return str.charAt(0).toUpperCase()+str.slice(1)}}])})();(function(){"use strict";var module=angular.module("onsen");function cleanClassAttribute(element){var classList=(""+element.attr("class")).split(/ +/).filter(function(classString){return classString!=="fa"&&classString.substring(0,3)!=="fa-"});element.attr("class",classList.join(" "))}function buildClassAndStyle(attrs){var classList=["ons-icon"];var style={};if(attrs.icon.indexOf("ion-")===0){classList.push(attrs.icon);classList.push("ons-icon--ion")}else if(attrs.icon.indexOf("fa-")===0){classList.push(attrs.icon);classList.push("fa")}else{classList.push("fa");classList.push("fa-"+attrs.icon)}var size=""+attrs.size;if(size.match(/^[1-5]x|lg$/)){classList.push("fa-"+size)}else if(typeof attrs.size==="string"){style["font-size"]=size}else{classList.push("fa-lg")}if(attrs.rotate==="90"||attrs.rotate==="180"||attrs.rotate==="270"){classList.push("ons-icon--rotate-"+attrs.rotate)}if(attrs.flip==="horizontal"||attrs.flip==="vertical"){classList.push("ons-icon--flip-"+attrs.flip)}if(attrs.fixedWidth!=="false"){classList.push("ons-icon--fw")}if(attrs.spin==="true"){classList.push("ons-icon--spin")}return{"class":classList.join(" "),style:style}}module.directive("onsIcon",["$onsen",function($onsen){return{restrict:"E",replace:false,transclude:false,link:function(scope,element,attrs){if(attrs.ngController){throw new Error("This element can't accept ng-controller directive.")}var update=function(){cleanClassAttribute(element);var builded=buildClassAndStyle(attrs);element.css(builded.style);element.addClass(builded["class"])};var builded=buildClassAndStyle(attrs);element.css(builded.style);element.addClass(builded["class"]);attrs.$observe("icon",update);attrs.$observe("size",update);attrs.$observe("fixedWidth",update);attrs.$observe("rotate",update);attrs.$observe("flip",update);attrs.$observe("spin",update);$onsen.cleaner.onDestroy(scope,function(){$onsen.clearComponent({scope:scope,element:element,attrs:attrs});element=scope=attrs=null})}}}])})();(function(){"use strict";var module=angular.module("onsen");module.directive("onsIfOrientation",["$onsen",function($onsen){return{restrict:"A",replace:false,transclude:false,scope:false,compile:function(element){element.css("display","none");return function(scope,element,attrs){element.addClass("ons-if-orientation-inner");window.addEventListener("orientationchange",update,false);window.addEventListener("resize",update,false);attrs.$observe("onsIfOrientation",update);update();$onsen.cleaner.onDestroy(scope,function(){window.removeEventListener("orientationchange",update,false);window.removeEventListener("resize",update,false);$onsen.clearComponent({element:element,scope:scope,attrs:attrs});element=scope=attrs=null});function update(){var userOrientation=(""+attrs.onsIfOrientation).toLowerCase();var orientation=getLandscapeOrPortraitFromInteger(window.orientation);if(userOrientation&&(userOrientation==="portrait"||userOrientation==="landscape")){if(userOrientation===orientation){element.css("display","block")}else{element.css("display","none")}}}function getLandscapeOrPortraitFromInteger(orientation){if(orientation===undefined){return window.innerWidth>window.innerHeight?"landscape":"portrait"}if(orientation==90||orientation==-90){return"landscape"}if(orientation===0||orientation==180){return"portrait"}}}}}}])})();(function(){"use strict";var module=angular.module("onsen");module.directive("onsIfPlatform",["$onsen",function($onsen){return{restrict:"A",replace:false,transclude:false,scope:false,compile:function(element){element.addClass("ons-if-platform-inner");element.css("display","none");var platform=getPlatformString();return function(scope,element,attrs){attrs.$observe("onsIfPlatform",function(userPlatform){if(userPlatform){update()}});update();$onsen.cleaner.onDestroy(scope,function(){$onsen.clearComponent({element:element,scope:scope,attrs:attrs});element=scope=attrs=null});function update(){if(attrs.onsIfPlatform.toLowerCase()===platform.toLowerCase()){element.css("display","block")}else{element.css("display","none")}}};function getPlatformString(){if(navigator.userAgent.match(/Android/i)){return"android"}if(navigator.userAgent.match(/BlackBerry/i)||navigator.userAgent.match(/RIM Tablet OS/i)||navigator.userAgent.match(/BB10/i)){return"blackberry"}if(navigator.userAgent.match(/iPhone|iPad|iPod/i)){return"ios"}if(navigator.userAgent.match(/IEMobile/i)){return"windows"}var isOpera=!!window.opera||navigator.userAgent.indexOf(" OPR/")>=0;if(isOpera){return"opera"}var isFirefox=typeof InstallTrigger!=="undefined";if(isFirefox){return"firefox"}var isSafari=Object.prototype.toString.call(window.HTMLElement).indexOf("Constructor")>0;if(isSafari){return"safari"}var isChrome=!!window.chrome&&!isOpera;if(isChrome){return"chrome"}var isIE=false||!!document.documentMode;if(isIE){return"ie"}return"unknown"}}}}])})();(function(){"use strict";var module=angular.module("onsen");module.directive("onsList",["$onsen",function($onsen){return{restrict:"E",scope:false,replace:false,transclude:false,compile:function(element,attrs){var templater=$onsen.generateModifierTemplater(attrs);element.addClass("list ons-list-inner");element.addClass(templater("list--*"))}}}])})();(function(){"use strict";var module=angular.module("onsen");module.directive("onsListHeader",["$onsen",function($onsen){return{restrict:"E",replace:false,transclude:false,compile:function(elem,attrs,transcludeFn){var templater=$onsen.generateModifierTemplater(attrs);elem.addClass("list__header ons-list-header-inner");elem.addClass(templater("list__header--*"))}}}])})();(function(){"use strict";var module=angular.module("onsen");module.directive("onsListItem",["$onsen",function($onsen){return{restrict:"E",replace:false,transclude:false,compile:function(elem,attrs,transcludeFn){var templater=$onsen.generateModifierTemplater(attrs);elem.addClass("list__item ons-list-item-inner");elem.addClass(templater("list__item--*"))}}}])})();(function(){"use strict";var module=angular.module("onsen");module.directive("onsModal",["$onsen","ModalView",function($onsen,ModalView){return{restrict:"E",replace:false,scope:false,transclude:false,compile:function(element,attrs){compile(element,attrs);return{pre:function(scope,element,attrs){var page=element.inheritedData("ons-page");if(page){page.registerExtraElement(element)}var modal=new ModalView(scope,element);$onsen.declareVarAttribute(attrs,modal);$onsen.aliasStack.register("ons.modal",modal);element.data("ons-modal",modal);scope.$on("$destroy",function(){element.data("ons-modal",undefined);$onsen.aliasStack.unregister("ons.modal",modal)})},post:function(){}}}};function compile(element,attrs){var modifierTemplater=$onsen.generateModifierTemplater(attrs);var html=element[0].innerHTML;element[0].innerHTML="";var wrapper=angular.element("<div></div>");wrapper.addClass("modal__content");wrapper.addClass(modifierTemplater("modal--*__content"));element.css("display","none");element.addClass("modal");element.addClass(modifierTemplater("modal--*"));wrapper[0].innerHTML=html;element.append(wrapper)}}])})();(function(){"use strict";var module=angular.module("onsen");module.directive("onsNavigator",["$compile","NavigatorView","$onsen",function($compile,NavigatorView,$onsen){return{restrict:"E",transclude:false,scope:true,compile:function(element){var html=$onsen.normalizePageHTML(element.html());element.contents().remove();return{pre:function(scope,element,attrs,controller){var navigator=new NavigatorView({scope:scope,element:element});$onsen.declareVarAttribute(attrs,navigator);if(attrs.page){navigator.pushPage(attrs.page,{})}else{var pageScope=navigator._createPageScope();var pageElement=angular.element(html);var linkScope=$compile(pageElement);var link=function(){linkScope(pageScope)};navigator._pushPageDOM("",pageElement,link,pageScope,{});pageElement=null}$onsen.aliasStack.register("ons.navigator",navigator);element.data("ons-navigator",navigator);scope.$on("$destroy",function(){element.data("ons-navigator",undefined);$onsen.aliasStack.unregister("ons.navigator",navigator);element=null})}}}}}])})();(function(){"use strict";var module=angular.module("onsen");module.directive("onsPage",["$onsen","PageView",function($onsen,PageView){function firePageInitEvent(element){var i=0;var f=function(){if(i++<5){if(isAttached(element)){fireActualPageInitEvent(element)}else{setImmediate(f)}}else{throw new Error('Fail to fire "pageinit" event. Attach "ons-page" element to the document after initialization.')}};f()}function fireActualPageInitEvent(element){var event=document.createEvent("HTMLEvents");event.initEvent("pageinit",true,true);element.dispatchEvent(event)}function isAttached(element){if(document.documentElement===element){return true}return element.parentNode?isAttached(element.parentNode):false}function preLink(scope,element,attrs,controller,transclude){var page=new PageView(scope,element,attrs);$onsen.declareVarAttribute(attrs,page);$onsen.aliasStack.register("ons.page",page);element.data("ons-page",page);var modifierTemplater=$onsen.generateModifierTemplater(attrs);element.addClass("page "+modifierTemplater("page--*"));var pageContent=angular.element(element[0].querySelector(".page__content"));pageContent.addClass(modifierTemplater("page--*__content"));pageContent=null;$onsen.cleaner.onDestroy(scope,function(){element.data("ons-page",undefined);$onsen.aliasStack.unregister("ons.page",page);$onsen.clearComponent({element:element,scope:scope,attrs:attrs});scope=element=attrs=null})}function postLink(scope,element,attrs){firePageInitEvent(element[0])}return{restrict:"E",transclude:false,scope:true,compile:function(element){var children=element.children().remove();if($onsen.isWebView()&&$onsen.isIOS7Above()){var fill=angular.element(document.createElement("div"));fill.addClass("page__status-bar-fill");fill.css({width:"0px",height:"0px"});element.prepend(fill)}var content=angular.element('<div class="page__content ons-page-inner"></div>').append(children);if(element.attr("style")){content.attr("style",element.attr("style"));element.attr("style","")}if(Modernizr.csstransforms3d){element.append(content)}else{content.css("overflow","visible");var wrapper=angular.element("<div></div>");wrapper.append(children);content.append(wrapper);element.append(content);wrapper=null;var scroller=new IScroll(content[0],{momentum:true,bounce:true,hScrollbar:false,vScrollbar:false,preventDefault:false});var offset=10;scroller.on("scrollStart",function(e){var scrolled=scroller.y-offset;if(scrolled<scroller.maxScrollY+40){scroller.refresh()}})}content=null;children=null;return{pre:preLink,post:postLink}}}}])})();(function(){"use strict";var module=angular.module("onsen");module.directive("onsRow",["$onsen","$timeout",function($onsen,$timeout){return{restrict:"E",replace:false,transclude:false,scope:false,compile:function(element,attrs){element.addClass("row ons-row-inner");return function(scope,element,attrs){attrs.$observe("align",function(align){update()});update();function update(){var align=(""+attrs.align).trim();if(align==="top"||align==="center"||align==="bottom"){element.removeClass("row-bottom row-center row-top");element.addClass("row-"+align)}}}}}}])})();(function(){"use strict";var module=angular.module("onsen");var TransitionAnimator=Class.extend({push:function(enterPage,leavePage,callback){callback()},pop:function(enterPage,leavePage,callback){callback()}});var ModalTransitionAnimator=TransitionAnimator.extend({backgroundMask:angular.element('<div style="position: absolute; width: 100%;'+'height: 100%; background-color: black;"></div>'),push:function(enterPage,leavePage,callback){var mask=this.backgroundMask.remove();leavePage.pageElement[0].parentNode.insertBefore(mask[0],leavePage.pageElement[0]);animit.runAll(animit(mask[0]).wait(.4).queue(function(done){mask.remove();done()}),animit(enterPage.pageElement[0]).queue({transform:"translate3D(0, 100%, 0)"}).queue({transform:"translate3D(0, 0, 0)"},{duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).resetStyle().queue(function(done){callback();done()}),animit(leavePage.pageElement[0]).queue({transform:"translate3D(0, 0, 0)",opacity:1}).queue({transform:"translate3D(0, -10%, 0)",opacity:.9},{duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).resetStyle())},pop:function(enterPage,leavePage,callback){var mask=this.backgroundMask.remove();enterPage.pageElement[0].parentNode.insertBefore(mask[0],enterPage.pageElement[0]);animit.runAll(animit(mask[0]).wait(.4).queue(function(done){mask.remove();done()}),animit(enterPage.pageElement[0]).queue({transform:"translate3D(0, -10%, 0)",opacity:.9}).queue({transform:"translate3D(0, 0, 0)",opacity:1},{duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}).resetStyle().queue(function(done){callback();done()}),animit(leavePage.pageElement[0]).queue({transform:"translate3D(0, 0, 0)"}).queue({transform:"translate3D(0, 100%, 0)"},{duration:.4,timing:"cubic-bezier(.1, .7, .1, 1)"}))}});module.service("Screen",["$compile","$onsen",function($compile,$onsen){var TRANSITION_END="webkitTransitionEnd transitionend msTransitionEnd oTransitionEnd";var TRANSITION_START="webkitAnimationStart animationStart msAnimationStart oAnimationStart";var Screen=Class.extend({init:function(scope,element,attrs){this.screenItems=[];this.scope=scope;this.element=element;this.attrs=attrs;this._doorLock=new DoorLock;this.attachMethods();if(scope.page){this.resetToPage(scope.page)}},isEmpty:function(){return this.screenItems.length<1},compilePageEl:function(pageEl,pageScope){var compiledPage=$compile(pageEl)(pageScope);return compiledPage},createPageScope:function(){var pageScope=this.scope.$new();return pageScope},_presentPageDOM:function(pageUrl,compiledPage,pageScope,callback){callback=callback||function(){};var screenItem={pageUrl:pageUrl,pageElement:compiledPage,pageScope:pageScope,destroy:function(){this.pageElement.remove();this.pageScope.$destroy()}};compiledPage.css("z-index",0);this.screenItems.push(screenItem);if(this.screenItems.length>1){var enterPage=screenItem;var leavePage=this.screenItems[this.screenItems.length-2];(new ModalTransitionAnimator).push(enterPage,leavePage,function(){leavePage.pageElement.css({display:"none"});callback()});this.element.append(compiledPage)}else{this.element.append(compiledPage);callback()}},presentPage:function(page){var self=this;this._doorLock.waitUnlock(function(){var unlock=self._doorLock.lock();$onsen.getPageHTMLAsync(page).then(function(html){var pageContent=angular.element(html.trim());var pageScope=self.createPageScope();var compiledPage=self.compilePageEl(pageContent,pageScope);self._presentPageDOM(page,compiledPage,pageScope,unlock)},function(){unlock();throw new Error("Page is not found: "+page)})})},dismissPage:function(){if(this.screenItems.length<2){return}var self=this;this._doorLock.waitUnlock(function(){var unlock=self._doorLock.lock();var leavePage=self.screenItems.pop();var enterPage=self.screenItems[self.screenItems.length-1];enterPage.pageElement.css({display:"block"});(new ModalTransitionAnimator).pop(enterPage,leavePage,function(){leavePage.destroy();unlock()})})},resetToPage:function(page){this.scope.presentPage(page);for(var i=0;i<this.screenItems.length-1;i++){this.screenItems[i].destroy()}},attachMethods:function(){this.scope.presentPage=this.presentPage.bind(this);this.scope.resetToPage=this.resetToPage.bind(this);this.scope.dismissPage=this.dismissPage.bind(this)}});return Screen}]);module.directive("onsScreen",["$compile","Screen","$onsen",function($compile,Screen,$onsen){return{restrict:"E",replace:false,transclude:false,scope:true,compile:function(element,attrs,transclude){var html=$onsen.normalizePageHTML(element.html().trim());element.contents().remove();return function(scope,element,attrs){var screen=new Screen(scope,element,attrs);$onsen.declareVarAttribute(attrs,screen);if(!attrs.page){var pageScope=screen.createPageScope();var compiled=$compile(angular.element(html))(pageScope);screen._presentPageDOM("",compiled,pageScope)}$onsen.aliasStack.register("ons.screen",screen);element.data("ons-screen",screen);scope.$on("$destroy",function(){element.data("ons-screen",undefined);$onsen.aliasStack.register("ons.screen",screen)})}}}}])})();(function(){"use strict";var module=angular.module("onsen");module.directive("onsScroller",["$onsen","$timeout",function($onsen,$timeout){return{restrict:"E",replace:false,transclude:true,scope:{onScrolled:"&",infinitScrollEnable:"="},compile:function(element,attrs){var content=element.addClass("ons-scroller").children().remove();var wrapper=angular.element("<div></div>");wrapper.addClass("ons-scroller__content ons-scroller-inner");element.append(wrapper);return function(scope,element,attrs,controller,transclude){if(attrs.ngController){throw new Error('"ons-scroller" can\'t accept "ng-controller" directive.')}transclude(scope.$parent,function(cloned){wrapper.append(cloned);wrapper=null});var scrollWrapper;scrollWrapper=element[0];var offset=parseInt(attrs.threshold)||10;if(scope.onScrolled){scrollWrapper.addEventListener("scroll",function(){if(scope.infinitScrollEnable){var scrollTopAndOffsetHeight=scrollWrapper.scrollTop+scrollWrapper.offsetHeight;var scrollHeightMinusOffset=scrollWrapper.scrollHeight-offset;if(scrollTopAndOffsetHeight>=scrollHeightMinusOffset){scope.onScrolled()}}})}if(!Modernizr.csstransforms3d){$timeout(function(){var iScroll=new IScroll(scrollWrapper,{momentum:true,bounce:true,hScrollbar:false,vScrollbar:false,preventDefault:false});iScroll.on("scrollStart",function(e){var scrolled=iScroll.y-offset;if(scrolled<iScroll.maxScrollY+40){iScroll.refresh()}});if(scope.onScrolled){iScroll.on("scrollEnd",function(e){var scrolled=iScroll.y-offset;if(scrolled<iScroll.maxScrollY){scope.onScrolled()}})}},500)}}}}}])})();(function(){"use strict";var module=angular.module("onsen");module.directive("onsSlidingMenu",["$compile","SlidingMenuView","$onsen",function($compile,SlidingMenuView,$onsen){return{restrict:"E",replace:false,transclude:false,scope:true,templateUrl:$onsen.DIRECTIVE_TEMPLATE_URL+"/sliding_menu.tpl",link:function(scope,element,attrs){if(attrs.ngController){throw new Error("This element can't accept ng-controller directive.")}var slidingMenu=new SlidingMenuView(scope,element,attrs);$onsen.aliasStack.register("ons.slidingMenu",slidingMenu);$onsen.declareVarAttribute(attrs,slidingMenu);element.data("ons-sliding-menu",slidingMenu);scope.$on("$destroy",function(){element.data("ons-sliding-menu",undefined);$onsen.aliasStack.unregister("ons.slidingMenu",slidingMenu)})}}}])})();(function(){"use strict";var module=angular.module("onsen");module.directive("onsSplitView",["$compile","SplitView","$onsen",function($compile,SplitView,$onsen){return{restrict:"E",replace:false,transclude:false,scope:{secondaryPage:"@",mainPage:"@",collapse:"@",swipable:"@",mainPageWidth:"@"},templateUrl:$onsen.DIRECTIVE_TEMPLATE_URL+"/split_view.tpl",link:function(scope,element,attrs){if(attrs.ngController){throw new Error("This element can't accept ng-controller directive.")}var splitView=new SplitView(scope,element,attrs);$onsen.declareVarAttribute(attrs,splitView);element.data("ons-split-view",splitView);
$onsen.aliasStack.register("ons.splitView",splitView);scope.$on("$destroy",function(){element.data("ons-split-view",undefined);$onsen.aliasStack.unregister("ons.splitView",splitView)})}}}])})();(function(){"use strict";var module=angular.module("onsen");module.directive("onsSwitch",["$onsen","SwitchView",function($onsen,SwitchView){return{restrict:"E",replace:false,transclude:false,scope:true,templateUrl:$onsen.DIRECTIVE_TEMPLATE_URL+"/switch.tpl",compile:function(element){return function(scope,element,attrs){if(attrs.ngController){throw new Error("This element can't accept ng-controller directive.")}var switchView=new SwitchView(element,scope,attrs);var checkbox=angular.element(element[0].querySelector("input[type=checkbox]"));scope.modifierTemplater=$onsen.generateModifierTemplater(attrs);attrs.$observe("checked",function(checked){scope.model=!!element.attr("checked")});attrs.$observe("name",function(name){if(!!element.attr("name")){checkbox.attr("name",name)}});if(attrs.ngModel){scope.$parent.$watch(attrs.ngModel,function(value){scope.model=value});scope.$watch("model",function(model){scope.$parent[attrs.ngModel]=model});scope.$parent[attrs.ngModel]=!!element.attr("checked")}$onsen.declareVarAttribute(attrs,switchView);element.data("ons-switch",switchView);$onsen.aliasStack.register("ons.switch",switchView);$onsen.cleaner.onDestroy(scope,function(){element.data("ons-switch",undefined);$onsen.aliasStack.unregister("ons.switch",switchView);$onsen.clearComponent({element:element,scope:scope,attrs:attrs});checkbox=element=attrs=scope=null})}}}}])})();(function(){"use strict";var module=angular.module("onsen");module.directive("onsTabbar",["$onsen","$compile","TabbarView",function($onsen,$compile,TabbarView){return{restrict:"E",replace:false,transclude:true,scope:{hide:"@",onActiveTabChanged:"&"},templateUrl:$onsen.DIRECTIVE_TEMPLATE_URL+"/tab_bar.tpl",link:function(scope,element,attrs,controller,transclude){if(attrs.ngController){throw new Error("This element can't accept ng-controller directive.")}scope.modifierTemplater=$onsen.generateModifierTemplater(attrs);scope.selectedTabItem={source:""};attrs.$observe("hideTabs",function(hide){scope.hideTabs=hide;tabbarView._onTabbarVisibilityChanged()});var tabbarView=new TabbarView(scope,element,attrs);scope.tabbarId=tabbarView._tabbarId;$onsen.aliasStack.register("ons.tabbar",tabbarView);element.data("ons-tabbar",tabbarView);$onsen.declareVarAttribute(attrs,tabbarView);transclude(function(cloned){angular.element(element[0].querySelector(".ons-tabbar-inner")).append(cloned)});scope.$on("$destroy",function(){element.data("ons-tabbar",undefined);$onsen.aliasStack.unregister("ons.tabbar",tabbarView)})}}}])})();(function(){"use strict";var module=angular.module("onsen");module.directive("onsTabbarItem",["$onsen",function($onsen){return{restrict:"E",replace:true,transclude:true,scope:{page:"@",active:"@",icon:"@",activeIcon:"@",label:"@"},templateUrl:$onsen.DIRECTIVE_TEMPLATE_URL+"/tab_bar_item.tpl",link:function(scope,element,attrs){var tabbarView=element.inheritedData("ons-tabbar");if(!tabbarView){throw new Error("ons-tabbar-item is must be child of ons-tabbar element.")}var radioButton=element[0].querySelector("input");scope.tabbarModifierTemplater=tabbarView.modifierTemplater;scope.modifierTemplater=$onsen.generateModifierTemplater(attrs);scope.tabbarId=tabbarView._tabbarId;scope.tabIcon=scope.icon;tabbarView.addTabItem(scope);scope.setActive=function(){element.addClass("active");radioButton.checked=true;if(scope.activeIcon){scope.tabIcon=scope.activeIcon}};scope.setInactive=function(){element.removeClass("active");radioButton.checked=false;scope.tabIcon=scope.icon};scope.isActive=function(){return element.hasClass("active")};scope.tryToChange=function(){tabbarView.setActiveTab(tabbarView._tabItems.indexOf(scope))};if(scope.active){tabbarView.setActiveTab(tabbarView._tabItems.indexOf(scope))}}}}])})();(function(){"use strict";var module=angular.module("onsen");module.directive("onsTemplate",["$onsen","$templateCache",function($onsen,$templateCache){return{restrict:"E",transclude:false,priority:1e3,terminal:true,compile:function(element){$templateCache.put(element.attr("id"),element.remove().html())}}}])})();(function(){"use strict";var module=angular.module("onsen");function ensureLeftContainer(element){var container=element[0].querySelector(".left");if(!container){container=document.createElement("div");container.setAttribute("class","left");container.innerHTML="&nbsp;"}if(container.innerHTML.trim()===""){container.innerHTML="&nbsp;"}angular.element(container).addClass("navigation-bar__left");return container}function ensureCenterContainer(element){var container=element[0].querySelector(".center");if(!container){container=document.createElement("div");container.setAttribute("class","center")}if(container.innerHTML.trim()===""){container.innerHTML="&nbsp;"}angular.element(container).addClass("navigation-bar__title navigation-bar__center");return container}function ensureRightContainer(element){var container=element[0].querySelector(".right");if(!container){container=document.createElement("div");container.setAttribute("class","right");container.innerHTML="&nbsp;"}if(container.innerHTML.trim()===""){container.innerHTML="&nbsp;"}angular.element(container).addClass("navigation-bar__right");return container}function hasCenterClassElementOnly(element){var hasCenter=false;var hasOther=false;var child,children=element.contents();for(var i=0;i<children.length;i++){child=angular.element(children[i]);if(child.hasClass("center")){hasCenter=true;continue}if(child.hasClass("left")||child.hasClass("right")){hasOther=true;continue}}return hasCenter&&!hasOther}function ensureToolbarItemElements(element){var center;if(hasCenterClassElementOnly(element)){center=ensureCenterContainer(element);element.contents().remove();element.append(center)}else{center=ensureCenterContainer(element);var left=ensureLeftContainer(element);var right=ensureRightContainer(element);element.contents().remove();element.append(angular.element([left,center,right]))}}module.directive("onsToolbar",["$onsen",function($onsen){return{restrict:"E",replace:false,scope:true,transclude:false,compile:function(element,attrs){var modifierTemplater=$onsen.generateModifierTemplater(attrs);element.addClass("navigation-bar");element.addClass(modifierTemplater("navigation-bar--*"));element.css({position:"absolute","z-index":"10000",left:"0px",right:"0px",top:"0px"});ensureToolbarItemElements(element);return{pre:function(scope,element,attrs){var pageView=element.inheritedData("ons-page");if(pageView){pageView.registerToolbar(element)}}}}}}])})();(function(){"use strict";var module=angular.module("onsen");module.directive("onsToolbarButton",["$onsen",function($onsen){return{restrict:"E",transclude:true,templateUrl:$onsen.DIRECTIVE_TEMPLATE_URL+"/toolbar_button.tpl",link:{pre:function(scope,element,attrs){if(attrs.ngController){throw new Error("This element can't accept ng-controller directive.")}scope.modifierTemplater=$onsen.generateModifierTemplater(attrs);$onsen.cleaner.onDestroy(scope,function(){$onsen.clearComponent({scope:scope,attrs:attrs,element:element});scope=element=attrs=null})}}}}])})();(function(){"use strict";var module=angular.module("onsen");var ComponentCleaner={decomposeNode:function(element){var children=element.remove().children();for(var i=0;i<children.length;i++){ComponentCleaner.decomposeNode(angular.element(children[i]))}},destroyAttributes:function(attrs){attrs.$$element=null;attrs.$$observers=null},destroyElement:function(element){element.remove()},destroyScope:function(scope){scope.$$listeners={};scope.$$watchers=null;scope=null},onDestroy:function(scope,fn){var clear=scope.$on("$destroy",function(){clear();fn.apply(null,arguments)})}};module.factory("ComponentCleaner",function(){return ComponentCleaner});(function(){var ngEventDirectives={};"click dblclick mousedown mouseup mouseover mouseout mousemove mouseenter mouseleave keydown keyup keypress submit focus blur copy cut paste".split(" ").forEach(function(name){var directiveName=directiveNormalize("ng-"+name);ngEventDirectives[directiveName]=["$parse",function($parse){return{compile:function($element,attr){var fn=$parse(attr[directiveName]);return function(scope,element,attr){var listener=function(event){scope.$apply(function(){fn(scope,{$event:event})})};element.on(name,listener);ComponentCleaner.onDestroy(scope,function(){element.off(name,listener);element=null;ComponentCleaner.destroyScope(scope);scope=null;ComponentCleaner.destroyAttributes(attr);attr=null})}}}}];function directiveNormalize(name){return name.replace(/-([a-z])/g,function(matches){return matches[1].toUpperCase()})}});module.config(["$provide",function($provide){var shift=function($delegate){$delegate.shift();return $delegate};Object.keys(ngEventDirectives).forEach(function(directiveName){$provide.decorator(directiveName+"Directive",["$delegate",shift])})}]);Object.keys(ngEventDirectives).forEach(function(directiveName){module.directive(directiveName,ngEventDirectives[directiveName])})})()})();(function(){"use strict";var util={init:function(){this.ready=false},addBackButtonListener:function(fn){if(this._ready){window.document.addEventListener("backbutton",fn,false)}else{window.document.addEventListener("deviceready",function(){window.document.addEventListener("backbutton",fn,false)})}},removeBackButtonListener:function(fn){if(this._ready){window.document.removeEventListener("backbutton",fn,false)}else{window.document.addEventListener("deviceready",function(){window.document.removeEventListener("backbutton",fn,false)})}}};util.init();angular.module("onsen").service("DeviceBackButtonHandler",function(){this._init=function(){if(window.ons.isWebView()){window.document.addEventListener("deviceready",function(){util._ready=true},false)}else{util._ready=true}this._bindedCallback=this._callback.bind(this);this.enable()};this._isEnabled=false;this.enable=function(){if(!this._isEnabled){util.addBackButtonListener(this._bindedCallback);this._isEnabled=true}};this.disable=function(){if(this._isEnabled){util.removeBackButtonListener(this._bindedCallback);this._isEnabled=false}};this.fireDeviceBackButtonEvent=function(){var event=document.createEvent("Event");event.initEvent("backbutton",true,true);document.dispatchEvent(event)};this._callback=function(){this._dispatchDeviceBackButtonEvent()};this.create=function(element,callback){if(!(element instanceof angular.element().constructor)){throw new Error("element must be an instance of jqLite")}if(!(callback instanceof Function)){throw new Error("callback must be an instance of Function")}var handler={_callback:callback,_element:element,disable:function(){this._element.data("device-backbutton-handler",null)},setListener:function(callback){this._callback=callback},enable:function(){this._element.data("device-backbutton-handler",this)},isEnabled:function(){return this._element.data("device-backbutton-handler")===this},destroy:function(){this._element.data("device-backbutton-handler",null);this._callback=this._element=null}};handler.enable();return handler};this._dispatchDeviceBackButtonEvent=function(event){var tree=this._captureTree();var element=this._findHandlerLeafElement(tree);var handler=element.data("device-backbutton-handler");handler._callback(createEvent(element));function createEvent(element){return{_element:element,callParentHandler:function(){var parent=this._element.parent();var hander=null;while(parent[0]){if(handler=parent.data("device-backbutton-handler")){return handler._callback(createEvent(parent))}parent=parent.parent()}}}}};this._dumpParents=function(element){while(element[0]){console.log(element[0].nodeName.toLowerCase()+"."+element.attr("class"));element=element.parent()}};this._captureTree=function(){return createTree(angular.element(document.body));function createTree(element){return{element:element,children:Array.prototype.concat.apply([],Array.prototype.map.call(element.children(),function(child){child=angular.element(child);if(child[0].style.display==="none"){return[]}if(child.children().length===0&&!child.data("device-backbutton-handler")){return[]}var result=createTree(child);if(result.children.length===0&&!child.data("device-backbutton-handler")){return[]}return[result]}))}}};this._dumpTree=function(node){_dump(node,0);function _dump(node,level){var pad=new Array(level+1).join("  ");console.log(pad+node.element[0].nodeName.toLowerCase());node.children.forEach(function(node){_dump(node,level+1)})}};this._findHandlerLeafElement=function(tree){return find(tree);function find(node){if(node.children.length===0){return node.element}if(node.children.length===1){return find(node.children[0])}return node.children.map(function(childNode){return childNode.element}).reduce(function(left,right){if(left===null){return right}var leftZ=parseInt(window.getComputedStyle(left[0],"").zIndex,10);var rightZ=parseInt(window.getComputedStyle(right[0],"").zIndex,10);if(!isNaN(leftZ)&&!isNaN(rightZ)){return leftZ>rightZ?left:right}throw new Error("Capturing backbutton-handler is failure.")},null)}};this._init()})})();(function(){"use strict";var module=angular.module("onsen");module.factory("$onsen",["$rootScope","$window","$cacheFactory","$document","$templateCache","$http","$q","ComponentCleaner","DeviceBackButtonHandler",function($rootScope,$window,$cacheFactory,$document,$templateCache,$http,$q,ComponentCleaner,DeviceBackButtonHandler){var unlockerDict={_unlockersDict:{},_unlockedVarDict:{},_addVarLock:function(name,unlocker){if(!(unlocker instanceof Function)){throw new Error("unlocker argument must be an instance of Function.")}if(this._unlockersDict[name]){this._unlockersDict[name].push(unlocker)}else{this._unlockersDict[name]=[unlocker]}},unlockVarName:function(varName){var unlockers=this._unlockersDict[varName];if(unlockers){unlockers.forEach(function(unlock){unlock()})}this._unlockedVarDict[varName]=true},addCallback:function(dependencies,callback){if(!(callback instanceof Function)){throw new Error("callback argument must be an instance of Function.")}var doorLock=new DoorLock;var self=this;dependencies.forEach(function(varName){if(!self._unlockedVarDict[varName]){var unlock=doorLock.lock();self._addVarLock(varName,unlock)}});if(doorLock.isLocked()){doorLock.waitUnlock(callback)}else{callback()}}};var aliasStack={_stackDict:{},register:function(name,object){this._getStack(name).push(object);$onsen._defineVar(name,object)},unregister:function(name,target){var stack=this._getStack(name);var index=stack.indexOf(target);if(index===-1){throw new Error("no such object: "+target)}stack.splice(index,1);var obj=stack.length>0?stack[stack.length-1]:null;$onsen._defineVar(name,obj)},_getStack:function(name){if(!this._stackDict[name]){this._stackDict[name]=[]}return this._stackDict[name]}};var $onsen={DIRECTIVE_TEMPLATE_URL:"templates",aliasStack:aliasStack,cleaner:ComponentCleaner,DeviceBackButtonHandler:DeviceBackButtonHandler,_defaultDeviceBackButtonHandler:DeviceBackButtonHandler.create(angular.element(document.body),function(){navigator.app.exitApp()}),getDefaultDeviceBackButtonHandler:function(){return this._defaultDeviceBackButtonHandler},clearComponent:function(params){if(params.scope){ComponentCleaner.destroyScope(params.scope)}if(params.attrs){ComponentCleaner.destroyAttributes(params.attrs)}if(params.element){ComponentCleaner.destroyElement(params.element)}if(params.elements){params.elements.forEach(function(element){ComponentCleaner.destroyElement(element)})}},upTo:function(el,tagName){tagName=tagName.toLowerCase();do{if(!el){return null}el=el.parentNode;if(el.tagName.toLowerCase()==tagName){return el}}while(el.parentNode);return null},waitForVariables:function(dependencies,callback){unlockerDict.addCallback(dependencies,callback)},findElementeObject:function(element,name){return element.inheritedData(name)},getPageHTMLAsync:function(page){var cache=$templateCache.get(page);if(cache){var deferred=$q.defer();var html=typeof cache==="string"?cache:cache[1];deferred.resolve(this.normalizePageHTML(html));return deferred.promise}else{return $http({url:page,method:"GET"}).then(function(response){var html=response.data;return this.normalizePageHTML(html)}.bind(this))}},normalizePageHTML:function(html){html=(""+html).trim();if(!html.match(/^<ons-page/)){html="<ons-page>"+html+"</ons-page>"}return html},generateModifierTemplater:function(attrs){var modifiers=attrs&&typeof attrs.modifier==="string"?attrs.modifier.trim().split(/ +/):[];return function(template){return modifiers.map(function(modifier){return template.replace("*",modifier)}).join(" ")}},declareVarAttribute:function(attrs,object){if(typeof attrs["var"]==="string"){var varName=attrs["var"];this._defineVar(varName,object);unlockerDict.unlockVarName(varName)}},isAndroid:function(){return!!window.navigator.userAgent.match(/android/i)},isIOS:function(){return!!window.navigator.userAgent.match(/(ipad|iphone|ipod touch)/i)},isWebView:function(){return window.ons.isWebView()},isIOS7Above:function(){var ua=window.navigator.userAgent;var match=ua.match(/(iPad|iPhone|iPod touch);.*CPU.*OS (\d+)_(\d+)/i);var result=match?parseFloat(match[2]+"."+match[3])>=7:false;return function(){return result}}(),_defineVar:function(name,object){var names=name.split(/\./);function set(container,names,object){var name;for(var i=0;i<names.length-1;i++){name=names[i];if(container[name]===undefined||container[name]===null){container[name]={}}container=container[name]}container[names[names.length-1]]=object}set($window,names,object);set($rootScope,names,object)}};return $onsen}])})();window.animit=function(){"use strict";var Animit=function(element){if(!(this instanceof Animit)){return new Animit(element)}if(element instanceof HTMLElement){this.elements=[element]}else if(Object.prototype.toString.call(element)==="[object Array]"){this.elements=element}else{throw new Error("First argument must be an array or an instance of HTMLElement.")}this.transitionQueue=[];this.lastStyleAttributeDict=[];var self=this;this.elements.forEach(function(element,index){if(!element.hasAttribute("data-animit-orig-style")){self.lastStyleAttributeDict[index]=element.getAttribute("style");element.setAttribute("data-animit-orig-style",self.lastStyleAttributeDict[index]||"")}else{self.lastStyleAttributeDict[index]=element.getAttribute("data-animit-orig-style")}})};Animit.prototype={transitionQueue:undefined,element:undefined,play:function(callback){if(typeof callback==="function"){this.transitionQueue.push(function(done){callback();done()})}this.startAnimation();return this},queue:function(transition,options){var queue=this.transitionQueue;if(transition&&options){options.css=transition;transition=new Animit.Transition(options)}if(!(transition instanceof Function||transition instanceof Animit.Transition)){if(transition.css){transition=new Animit.Transition(transition)}else{transition=new Animit.Transition({css:transition})}}if(transition instanceof Function){queue.push(transition)}else if(transition instanceof Animit.Transition){queue.push(transition.build())}else{throw new Error("Invalid arguments")}return this},wait:function(seconds){var self=this;this.transitionQueue.push(function(done){setTimeout(done,1e3*seconds)});return this},resetStyle:function(options){options=options||{};var self=this;if(options.transition&&!options.duration){throw new Error('"options.duration" is required when "options.transition" is enabled.')}if(options.transition||options.duration&&options.duration>0){var transitionValue=options.transition||"all "+options.duration+"s "+(options.timing||"linear");var transitionStyle="transition: "+transitionValue+"; -"+Animit.prefix+"-transition: "+transitionValue+";";this.transitionQueue.push(function(done){var elements=this.elements;elements.forEach(function(element,index){element.style[Animit.prefix+"Transition"]=transitionValue;element.style.transition=transitionValue;var styleValue=(self.lastStyleAttributeDict[index]?self.lastStyleAttributeDict[index]+"; ":"")+transitionStyle;element.setAttribute("style",styleValue)});var removeListeners=util.addOnTransitionEnd(elements[0],function(){clearTimeout(timeoutId);reset();done()});var timeoutId=setTimeout(function(){removeListeners();reset();done()},options.duration*1e3*1.4)})}else{this.transitionQueue.push(function(done){reset();done()})}return this;function reset(){self.elements.forEach(function(element,index){element.style[Animit.prefix+"Transition"]="none";element.style.transition="none";if(self.lastStyleAttributeDict[index]){element.setAttribute("style",self.lastStyleAttributeDict[index])}else{element.setAttribute("style","");element.removeAttribute("style")}})}},startAnimation:function(){this._dequeueTransition();return this},_dequeueTransition:function(){var transition=this.transitionQueue.shift();if(this._currentTransition){throw new Error("Current transition exists.")}this._currentTransition=transition;var self=this;var called=false;var done=function(){if(!called){called=true;self._currentTransition=undefined;self._dequeueTransition()}else{throw new Error("Invalid state: This callback is called twice.")}};if(transition){transition.call(this,done)}}};Animit.cssPropertyDict=function(){var styles=window.getComputedStyle(document.documentElement,"");var dict={};var a="A".charCodeAt(0);var z="z".charCodeAt(0);for(var key in styles){if(styles.hasOwnProperty(key)){var char=key.charCodeAt(0);if(a<=key.charCodeAt(0)&&z>=key.charCodeAt(0)){if(key!=="cssText"&&key!=="parentText"&&key!=="length"){dict[key]=true}}}}return dict}();Animit.hasCssProperty=function(name){return!!Animit.cssPropertyDict[name]};Animit.prefix=function(){var styles=window.getComputedStyle(document.documentElement,""),pre=(Array.prototype.slice.call(styles).join("").match(/-(moz|webkit|ms)-/)||styles.OLink===""&&["","o"])[1];return pre}();Animit.runAll=function(){for(var i=0;i<arguments.length;i++){arguments[i].play()}};Animit.Transition=function(options){this.options=options||{};this.options.duration=this.options.duration||0;this.options.timing=this.options.timing||"linear";this.options.css=this.options.css||{};this.options.property=this.options.property||"all"};Animit.Transition.prototype={build:function(){if(Object.keys(this.options.css).length===0){throw new Error("options.css is required.")}var css=createActualCssProps(this.options.css);if(this.options.duration>0){var transitionValue=util.buildTransitionValue(this.options);var self=this;return function(callback){var elements=this.elements;var timeout=self.options.duration*1e3*1.4;var removeListeners=util.addOnTransitionEnd(elements[0],function(){clearTimeout(timeoutId);callback()});var timeoutId=setTimeout(function(){removeListeners();callback()},timeout);elements.forEach(function(element,index){element.style[Animit.prefix+"Transition"]=transitionValue;element.style.transition=transitionValue;Object.keys(css).forEach(function(name){element.style[name]=css[name]})})}}if(this.options.duration<=0){return function(callback){var elements=this.elements;elements.forEach(function(element,index){element.style[Animit.prefix+"Transition"]="none";element.transition="none";Object.keys(css).forEach(function(name){element.style[name]=css[name]})});elements.forEach(function(element){element.getBoundingClientRect()});if(window.requestAnimationFrame){requestAnimationFrame(callback)}else{setTimeout(callback,1e3/30)}}}function createActualCssProps(css){var result={};Object.keys(css).forEach(function(name){var value=css[name];name=util.normalizeStyleName(name);var prefixed=Animit.prefix+util.capitalize(name);if(Animit.cssPropertyDict[name]){result[name]=value}else if(Animit.cssPropertyDict[prefixed]){result[prefixed]=value}else{result[prefixed]=value;result[name]=value}});return result}}};var util={normalizeStyleName:function(name){name=name.replace(/-[a-zA-Z]/g,function(all){return all.slice(1).toUpperCase()});return name.charAt(0).toLowerCase()+name.slice(1)},capitalize:function(str){return str.charAt(0).toUpperCase()+str.slice(1)},buildTransitionValue:function(params){params.property=params.property||"all";params.duration=params.duration||.4;params.timing=params.timing||"linear";var props=params.property.split(/ +/);return props.map(function(prop){return prop+" "+params.duration+"s "+params.timing}).join(", ")},addOnTransitionEnd:function(element,callback){if(!element){return function(){}}var fn=function(event){if(element==event.target){event.stopPropagation();removeListeners();callback()}};var removeListeners=function(){util._transitionEndEvents.forEach(function(eventName){element.removeEventListener(eventName,fn)})};util._transitionEndEvents.forEach(function(eventName){element.addEventListener(eventName,fn,false)});return removeListeners},_transitionEndEvents:function(){if(Animit.prefix==="webkit"||Animit.prefix==="o"||Animit.prefix==="moz"||Animit.prefix==="ms"){return[Animit.prefix+"TransitionEnd","transitionend"]}return["transitionend"]}()};return Animit}();(function(){"use strict";window.addEventListener("load",function(){FastClick.attach(document.body)},false);(new Viewport).setup();Modernizr.testStyles("#modernizr { -webkit-overflow-scrolling:touch }",function(elem,rule){Modernizr.addTest("overflowtouch",window.getComputedStyle&&window.getComputedStyle(elem).getPropertyValue("-webkit-overflow-scrolling")=="touch")})})();(function(){"use strict";angular.module("onsen").run(["$templateCache",function($templateCache){var templates=window.document.querySelectorAll('script[type="text/ons-template"]');for(var i=0;i<templates.length;i++){var template=angular.element(templates[i]);var id=template.attr("id");if(typeof id==="string"){$templateCache.put(id,template.text())}}}])})();
"use strict";(function(){var app=angular.module("ui.map",["ui.event"]);function bindMapEvents(scope,eventsStr,googleObject,element){angular.forEach(eventsStr.split(" "),function(eventName){window.google.maps.event.addListener(googleObject,eventName,function(event){element.triggerHandler("map-"+eventName,event);if(!scope.$$phase){scope.$apply()}})})}app.value("uiMapConfig",{}).directive("uiMap",["uiMapConfig","$parse",function(uiMapConfig,$parse){var mapEvents="bounds_changed center_changed click dblclick drag dragend "+"dragstart heading_changed idle maptypeid_changed mousemove mouseout "+"mouseover projection_changed resize rightclick tilesloaded tilt_changed "+"zoom_changed";var options=uiMapConfig||{};return{restrict:"A",link:function(scope,elm,attrs){var opts=angular.extend({},options,scope.$eval(attrs.uiOptions));var map=new window.google.maps.Map(elm[0],opts);var model=$parse(attrs.uiMap);model.assign(scope,map);bindMapEvents(scope,mapEvents,map,elm)}}}]);app.value("uiMapInfoWindowConfig",{}).directive("uiMapInfoWindow",["uiMapInfoWindowConfig","$parse","$compile",function(uiMapInfoWindowConfig,$parse,$compile){var infoWindowEvents="closeclick content_change domready "+"position_changed zindex_changed";var options=uiMapInfoWindowConfig||{};return{link:function(scope,elm,attrs){var opts=angular.extend({},options,scope.$eval(attrs.uiOptions));opts.content=elm[0];var model=$parse(attrs.uiMapInfoWindow);var infoWindow=model(scope);if(!infoWindow){infoWindow=new window.google.maps.InfoWindow(opts);model.assign(scope,infoWindow)}bindMapEvents(scope,infoWindowEvents,infoWindow,elm);elm.replaceWith("<div></div>");var _open=infoWindow.open;infoWindow.open=function open(a1,a2,a3,a4,a5,a6){$compile(elm.contents())(scope);_open.call(infoWindow,a1,a2,a3,a4,a5,a6)}}}}]);function mapOverlayDirective(directiveName,events){app.directive(directiveName,[function(){return{restrict:"A",link:function(scope,elm,attrs){scope.$watch(attrs[directiveName],function(newObject){if(newObject){bindMapEvents(scope,events,newObject,elm)}})}}}])}mapOverlayDirective("uiMapMarker","animation_changed click clickable_changed cursor_changed "+"dblclick drag dragend draggable_changed dragstart flat_changed icon_changed "+"mousedown mouseout mouseover mouseup position_changed rightclick "+"shadow_changed shape_changed title_changed visible_changed zindex_changed");mapOverlayDirective("uiMapPolyline","click dblclick mousedown mousemove mouseout mouseover mouseup rightclick");mapOverlayDirective("uiMapPolygon","click dblclick mousedown mousemove mouseout mouseover mouseup rightclick");mapOverlayDirective("uiMapRectangle","bounds_changed click dblclick mousedown mousemove mouseout mouseover "+"mouseup rightclick");mapOverlayDirective("uiMapCircle","center_changed click dblclick mousedown mousemove "+"mouseout mouseover mouseup radius_changed rightclick");mapOverlayDirective("uiMapGroundOverlay","click dblclick")})();
angular.module("angularLocalStorage",["ngCookies"]).factory("storage",["$parse","$cookieStore","$window","$log",function($parse,$cookieStore,$window,$log){var storage=typeof $window.localStorage==="undefined"?undefined:$window.localStorage;var supported=!(typeof storage==="undefined"||typeof $window.JSON==="undefined");var privateMethods={parseValue:function(res){var val;try{val=$window.JSON.parse(res);if(typeof val==="undefined"){val=res}if(val==="true"){val=true}if(val==="false"){val=false}if($window.parseFloat(val)===val&&!angular.isObject(val)){val=$window.parseFloat(val)}}catch(e){val=res}return val}};var publicMethods={set:function(key,value){if(!supported){try{$cookieStore.put(key,value);return value}catch(e){$log.log("Local Storage not supported, make sure you have angular-cookies enabled.")}}var saver=$window.JSON.stringify(value);storage.setItem(key,saver);return privateMethods.parseValue(saver)},get:function(key){if(!supported){try{return privateMethods.parseValue($.cookie(key))}catch(e){return null}}var item=storage.getItem(key);return privateMethods.parseValue(item)},remove:function(key){if(!supported){try{$cookieStore.remove(key);return true}catch(e){return false}}storage.removeItem(key);return true},bind:function($scope,key,opts){var defaultOpts={defaultValue:"",storeName:""};if(angular.isString(opts)){opts=angular.extend({},defaultOpts,{defaultValue:opts})}else{opts=angular.isUndefined(opts)?defaultOpts:angular.extend(defaultOpts,opts)}var storeName=opts.storeName||key;if(!publicMethods.get(storeName)){publicMethods.set(storeName,opts.defaultValue)}$parse(key).assign($scope,publicMethods.get(storeName));$scope.$watch(key,function(val){if(angular.isDefined(val)){publicMethods.set(storeName,val)}},true);return publicMethods.get(storeName)},unbind:function($scope,key,storeName){storeName=storeName||key;$parse(key).assign($scope,null);$scope.$watch(key,function(){});publicMethods.remove(storeName)},clearAll:function(){storage.clear()}};return publicMethods}]);